...
namespace POSRestaurant.Setting
{
    public partial class AddLayout : Form
    {
        ...
        public AddLayout(POSRestaurant.forms.MainForm frm)
        {
            InitializeComponent();
            this.editmode = 0;
            this.id = "";
        ...
        private void vButton2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Please Enter Row Height");
                    textBox1.Focus();
                    return;
                }
                ...
                {
                    int Num;
                    bool isNum = int.TryParse(textBox1.Text.ToString(), out Num); //c is your variable
                    if (isNum)
                    {
                ...
                if (editmode == 0)
                {
                    ...
                    q = "insert into Tablelayout (id,Rows,Columns,tablename,RowSize) values('" + id + "','" + txtrows.Text.Trim().Replace("'", "''") + "','" + txtcols.Text.Trim().Replace("'", "''") + "','" + comboBox1.Text + "','" + textBox1.Text + "')";
                ...
                if (editmode == 1)
                {
                    string q = "update Tablelayout set RowSize='"+textBox1.Text+"'  , Rows='" + txtrows.Text.Trim().Replace("'", "''") + "' , Columns ='" + txtcols.Text.Trim().Replace("'", "''") + "',tablename='" + comboBox1.Text + "' where id='" + id + "'";
                    objCore = new classes.Clsdbcon();
                    objCore.executeQuery(q);

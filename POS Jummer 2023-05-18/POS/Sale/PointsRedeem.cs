            ...
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        _frm.discountkeys(dis.ToString(),"Loyality","Points Redeem");
                        this.Close();
                    }
                }
            }
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                try
                {
                    ...
                    using (StreamReader readStream = new StreamReader(stream1, Encoding.UTF8))
                    {
                        ...
                        {
                            float prc = 0, sizeprice = 0,grossprice=0;
                            ...
                            try
                            {
                                ...
                                if (dsmenu.Tables[0].Rows.Count > 0)
                                {
                                    ...
                                    try
                                    {
                                        grossprice = float.Parse(dsmenu.Tables[0].Rows[0]["Grossprice"].ToString());
                                    }
                                    catch (Exception ex)
                                    {
                                ...
                                if (dsflavour.Tables[0].Rows.Count > 0)
                                {
                                  string  temp = dsflavour.Tables[0].Rows[0]["GrossPrice"].ToString();
                                    ...
                                    if (_frm.pricemethod.ToLower() == "gross")
                                    {
                                        temp = dsflavour.Tables[0].Rows[0]["GrossPrice"].ToString();
                                        if (temp == "")
                                        {
                                            temp = "0";
                                        }
                            ...
                            prc = prc + sizeprice;
                            _frm.fillgrid(menuid, "", size + name, "0", "1", "New", "", fid, "", "", kdsid, "", "", "", textBox1.Text,grossprice.ToString());

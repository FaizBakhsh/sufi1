...
{
    public partial class kdsnewfixmainrecall : Form
    {
        public kdsnewfixmainrecall()
        {
            InitializeComponent();
        }
        ...
        public void fillpending(string refresh)
        {
            ...
            string q = "";
            q = "SELECT DISTINCT TOP " + top + "  dbo.Sale.id as id, dbo.Sale.Customer, dbo.sale.time, dbo.sale.DeliveredTime, dbo.Sale.terminal, dbo.Sale.TerminalOrder, dbo.Saledetails.kdsgroup FROM  dbo.Saledetails INNER JOIN               dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id INNER JOIN              dbo.Sale ON dbo.Saledetails.saleid = dbo.Sale.Id WHERE ((dbo.Saledetails.OrderStatusmain = 'Completed')  AND (dbo.Saledetails.kdsgroup > 0) )  ORDER BY dbo.Sale.DeliveredTime desc";

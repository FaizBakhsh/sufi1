...
{
    public partial class frmComparativeBalaneSheet1 : Form
    {
        double ablnce = 0;
        ...
        public frmComparativeBalaneSheet1()
        {
            InitializeComponent();
        }
        ...
        public void bindreport()
        {
            ...
            try
            {
                ...
                POSRestaurant.Reports.Accounts.rptcmpblncesht rptDoc = new rptcmpblncesht();
                POSRestaurant.Reports.Accounts.dscompblncesht dsrpt = new dscompblncesht();
        ...
        public DataTable getAllOrders2()
        {
            ...
            string qr = "select top 3 * from branch";
            ...
            for (int z = 0; z < dsbranch.Tables[0].Rows.Count; z++)
            {
                ...
                for (int i = 0; i < dsaccounts.Tables[0].Rows.Count; i++)
                {
                    ...
                    getAllOrders1(dsaccounts.Tables[0].Rows[i]["id"].ToString(), "Assets", "Fixed Assets", dsaccounts.Tables[0].Rows[i]["Name"].ToString(), rem, dsbranch.Tables[0].Rows[z]["BranchName"].ToString());
                }
                ...
                for (int i = 0; i < dsaccounts.Tables[0].Rows.Count; i++)
                {
                    ...
                    getAllOrders1(dsaccounts.Tables[0].Rows[i]["id"].ToString(), "Assets", "Current Assets", dsaccounts.Tables[0].Rows[i]["Name"].ToString(), rem, dsbranch.Tables[0].Rows[z]["BranchName"].ToString());
                }
                ...
                for (int i = 0; i < dsaccounts.Tables[0].Rows.Count; i++)
                {
                    ...
                    getAllOrders1(dsaccounts.Tables[0].Rows[i]["id"].ToString(), "Liabilities", "Equity and Capital", dsaccounts.Tables[0].Rows[i]["Name"].ToString(), rem, dsbranch.Tables[0].Rows[z]["BranchName"].ToString());
                }
                dsaccounts = new System.Data.DataSet();
                ...
                for (int i = 0; i < dsaccounts.Tables[0].Rows.Count; i++)
                {
                    ...
                    getAllOrders1(dsaccounts.Tables[0].Rows[i]["id"].ToString(), "Liabilities", "Long Term Liabilities", dsaccounts.Tables[0].Rows[i]["Name"].ToString(), rem, dsbranch.Tables[0].Rows[z]["BranchName"].ToString());
                }
                dsaccounts = new System.Data.DataSet();
                ...
                for (int i = 0; i < dsaccounts.Tables[0].Rows.Count; i++)
                {
                    ...
                    getAllOrders1(dsaccounts.Tables[0].Rows[i]["id"].ToString(), "Liabilities", "Current Liabilities", dsaccounts.Tables[0].Rows[i]["Name"].ToString(), rem, dsbranch.Tables[0].Rows[z]["BranchName"].ToString());
                } 
            }
            return dtrpt;
        }
       
        public void getAllOrders1(string id , string type, string accounttype, string name, string count,string branch)
        {
                ...
                if (dtrpt.Rows.Count >= 1)
                {
                    if (count == "0")
                    {
                        ...
                        if (accounttype == "Equity and Capital")
                        {
                            ...
                            if (logo == "")
                            {
                                dtrpt.Rows.Add(branch,type, accounttype, "Profit/Loss ", netprofit, null);
                            }
                            else
                            {
                                dtrpt.Rows.Add(branch, type, accounttype, "Profit/Loss ", netprofit, dscompany.Tables[0].Rows[0]["logo"]);
                            }
                        }
                        if (logo == "")
                        {
                            dtrpt.Rows.Add(branch, type, accounttype, name,Convert.ToDouble( blnc.Replace("(","").Replace(")","")), null);
                        }
                        else
                        {
                            dtrpt.Rows.Add(branch, type, accounttype, name, Convert.ToDouble(blnc.Replace("(", "").Replace(")", "")), dscompany.Tables[0].Rows[0]["logo"]);
                        ...
                        if (logo == "")
                        {
                            dtrpt.Rows.Add(branch, type, accounttype, "Total " + accounttype, Convert.ToDouble(temp.Replace("(", "").Replace(")", "")), null);
                        }
                        else
                        {

                            dtrpt.Rows.Add(branch, type, accounttype, "Total " + accounttype, Convert.ToDouble(temp.Replace("(", "").Replace(")", "")), dscompany.Tables[0].Rows[0]["logo"]);
                        }
                        ...
                        if ((chk == true && accounttype == "Current Assets"))
                        {
                            ...
                            if (logo == "")
                            {
                                dtrpt.Rows.Add(branch, type,  "Total " + type,"", Convert.ToDouble(temp1.Replace("(", "").Replace(")", "")), null);
                       
                                //dtrpt.Rows.Add(branch, type, accounttype, "Total " + type, Convert.ToDouble(temp1.Replace("(", "").Replace(")", "")), null);
                            }
                            else
                            {
                                dtrpt.Rows.Add(branch, type,  "Total " + type,"", Convert.ToDouble(temp1.Replace("(", "").Replace(")", "")), dscompany.Tables[0].Rows[0]["logo"]);
                               // dtrpt.Rows.Add(branch, type, accounttype, "Total " + type, Convert.ToDouble(temp1.Replace("(", "").Replace(")", "")), dscompany.Tables[0].Rows[0]["logo"]);
        ...
        public void getprofitloss()
        {
            ...
            try
            {
                ...
                try
                {
                    q = "SELECT     sum(Credit) as Sales,sum(debit) as Salesret FROM         SalesAccount where (Date <=  '" + dateTimePicker1.Text + "')";
                    ds = objCore.funGetDataSet(q);
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                ...
                try
                {
                    q = "SELECT     sum(Debit) as Discount,sum(credit) as Discountret FROM         DiscountAccount where (Date <=  '" + dateTimePicker1.Text + "')";
                    ds = objCore.funGetDataSet(q);
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                ...
                try
                {
                    q = "SELECT     sum(Debit) as costsales,sum(credit) as costsalesrest FROM         CostSalesAccount where (Date <=  '" + dateTimePicker1.Text + "')";
                    ds = objCore.funGetDataSet(q);
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                ...
                try
                {
                    ...
                    {
                        q = "SELECT     sum(Debit) as opexp,sum(credit) as opexpret FROM         JournalAccount where PayableAccountId='" + dsexp.Tables[0].Rows[j][0].ToString() + "' and (Date <=  '" + dateTimePicker1.Text + "')";
                        ds = objCore.funGetDataSet(q);
                        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                        {
                ...
                try
                {
                    ...
                    for (int j = 0; j < dsexp.Tables[0].Rows.Count; j++)
                    {
                        q = "SELECT     sum(Debit) as opexp,sum(credit) as opexpres FROM         JournalAccount where PayableAccountId='" + dsexp.Tables[0].Rows[j][0].ToString() + "' and (Date <=  '" + dateTimePicker1.Text + "')";
                        ds = objCore.funGetDataSet(q);
                        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                        {
                ...
                //        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                //        {
                //            double sret = Convert.ToDouble(ds.Tables[0].Rows[i]["opexpret"].ToString());
                //            val = ds.Tables[0].Rows[i]["opexp"].ToString();
                //            if (val == "")
                //            {
                //                val = "0";

...
namespace POSRestaurant.RawItems
{
    public partial class Variance : Form
    {
        ...
        public void getdata()
        {
            ...
            ds.Columns.Add("RemainingQuantity", typeof(string));
            ds.Columns.Add("Physical Quantity", typeof(string));
        ...
        private void dataGridView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 3)
            {
                try
                {
                    ...
                    dataGridView1.Rows[e.RowIndex].Cells[4].Value = (physicalval -remaining).ToString();
                }
                catch (Exception ex)
                {
        ...
        private void vButton2_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow dr in dataGridView1.Rows)
            {
                ...
                if (dr.Cells[3].Value.ToString() != string.Empty && dr.Cells[3].Value.ToString() != "0")
                {
                    DataSet dss = new DataSet();
                    int id = 0;
                    dss = objcore.funGetDataSet("select max(id) as id from Variance");
                    if (dss.Tables[0].Rows.Count > 0)
                    {
                        string i = dss.Tables[0].Rows[0][0].ToString();
                        if (i == string.Empty)
                    ...
                    double variance = Math.Round(Convert.ToDouble(dr.Cells["variance"].Value.ToString()), 2);
                    //if (variance > 0)
                    //{
                    ...
                    //}
                    dss = new DataSet();
                    string q = "select * from Variance where itemid='" + dr.Cells["id"].Value.ToString() + "' and date='" + dateTimePicker1.Text + "'";
                    dss = objcore.funGetDataSet(q);
                    if (dss.Tables[0].Rows.Count > 0)
                    {
                        DialogResult dtr = MessageBox.Show("Item Variance Record of " + dr.Cells["ItemName"].Value.ToString() + " for this date already exist. Do you want to update it", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                        if (dtr == DialogResult.Yes)
                        {
                            double oldqty = Convert.ToDouble(dss.Tables[0].Rows[0]["quantity"].ToString());
                            variance = (Convert.ToDouble(dr.Cells["Physical Quantity"].Value.ToString())) - oldqty;
                            q = "update Variance set quantity='" + Math.Round(oldqty, 2).ToString() + "',physical='" + Math.Round(Convert.ToDouble(dr.Cells["Physical Quantity"].Value.ToString()), 2).ToString() + "',variance ='" + variance.ToString() + "' where   itemid='" + dr.Cells["id"].Value.ToString() + "' and date='" + dateTimePicker1.Text + "'";
                            objcore.executeQuery(q);
                            chk1 = true;
                        }                        
                    }
                    ...
                    {
                        q = "insert into Variance (id,itemid,date,quantity,physical,variance) values('" + id + "','" + dr.Cells["id"].Value.ToString() + "','" + dateTimePicker1.Text + "','" + Math.Round(Convert.ToDouble(dr.Cells["RemainingQuantity"].Value.ToString()), 2).ToString() + "','" + Math.Round(Convert.ToDouble(dr.Cells["Physical Quantity"].Value.ToString()), 2).ToString() + "','" + variance.ToString() + "')";
                        objcore.executeQuery(q);
                        chk = true;
                    }

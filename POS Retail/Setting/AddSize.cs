...
namespace POSRetail.Setting
{
    public partial class Addsize : Form
    {
         ...
         public Addsize(POSRetail.forms.MainForm frm)
        {
            InitializeComponent();
            this.editmode = 0;
            this.id = "";
        ...
        private void vButton2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (editmode == 1)
                {
                    string q = "update Size set SizeCode='" + txtName.Text.Trim().Replace("'", "''") + "' , SizeName ='" + txtdescription.Text.Trim().Replace("'", "''") + "' where id='" + id + "'";
                    POSRetail.classes.Clsdbcon objCore = new classes.Clsdbcon();
                    objCore.executeQuery(q);

...
namespace POSRestaurant.Reports
{
    public partial class frmSalesTill : Form
    {
        ...
        public DataTable getAllOrders()
        {
            ...
            double gross = 0, cashgst = 0, visagst = 0,posfee=0, gst = 0, cashdis = 0, carddis = 0, discount = 0, net = 0, cashrecv = 0, cardrecv = 0, cash = 0, credit = 0, master = 0, recvble = 0, service = 0, dinin = 0, takeaway = 0, delivery = 0, refund = 0, voidsale = 0, carhope = 0, calculatedcash = 0, drawerfloat = 0, bankingtotal = 0, declared = 0, over = 0, total = 0;
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (comboBox1.Text == "All")
                    {
                        q = "SELECT     SUM(TotalBill) AS gross, SUM(GST) AS gst,SUM(servicecharges) AS service, SUM(TotalBill) AS netsale, SUM(DiscountAmount) AS discount, SUM(POSFee) AS POSFee FROM         Sale where  (Date between '" + dateTimePicker1.Text + "' and '" + dateTimePicker2.Text + "')  and billstatus='Paid'";
                    }
                    else
                    {
                        q = "SELECT     SUM(TotalBill) AS gross, SUM(GST) AS gst,SUM(servicecharges) AS service, SUM(TotalBill) AS netsale, SUM(DiscountAmount) AS discount, SUM(POSFee) AS POSFee FROM         Sale where userid='"+comboBox1.SelectedValue+"' and (Date between '" + dateTimePicker1.Text + "' and '" + dateTimePicker2.Text + "')  and billstatus='Paid'";
                    }
                        ds = objcore.funGetDataSet(q);
        ...
        private void PrintReceiptshiftend(BinaryWriter bw, string cashier, string date, string shift, string type)
        {
           ...
           // bw.NormalFont(comp);
            ...
            //}
            double net = 0, gst = 0, cashdis = 0,posfee=0, carddis = 0, cashrecv = 0, cardrecv = 0, dis = 0, refund = 0, voidsale = 0, gross = 0, cashgst = 0, cardgst = 0, avgdinein = 0, avgdineintable = 0;

...
{
    public partial class Frmpnl2 : Form
    {
        ...
        public void bindreport()
        {
            ...
            try
            {
                ...
                rptDoc.SetParameterValue("report", "Comparative Profit n Loss");
                rptDoc.SetParameterValue("range", "");
                ...
                rptDoc.SetParameterValue("date2",  dateTimePicker3.Text + " to " + dateTimePicker4.Text);
        ...
        public DataTable getAllOrders2()
        {
            ...
            try
            {
                ...
                double grosmargin = 0;
                DataSet ds = new DataSet();
                string month1 = dateTimePicker3.Text.Substring(0, 2);
                string year1 = dateTimePicker3.Text.Substring(3);
                string month2 = dateTimePicker4.Text.Substring(0, 2);
                string year2 = dateTimePicker4.Text.Substring(3);
                string q = "";
                q = "select sum(TotalBill) as gross, sum(discountamount) as dis,sum(gst) as gst,avg(gstperc) as gstprc,avg(Discount) as Discount from sale where  date between '" + dateTimePicker3.Text + "' and '" + dateTimePicker4.Text + "' and billstatus='Paid'";
                // q = "select sum(TotalBill) as gross, sum(discountamount) as dis,sum(gst) as gst,avg(gstperc) as gstprc,avg(Discount) as Discount from sale where  MONTH(date) BETWEEN MONTH(" + month1 + ") AND MONTH(" + month2 + ") and YEAR(date) BETWEEN YEAR("+year1+") AND YEAR("+year2+") and billstatus='Paid'";
                ds = objCore.funGetDataSet(q);
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    q = "SELECT        TOP (100) PERCENT SUM(dbo.Saledetails.Price - dbo.Saledetails.Itemdiscount) AS price, SUM(dbo.Saledetails.Quantity) AS qty, SUM(dbo.Saledetails.Price) as menuprice, dbo.MenuItem.Price AS mprice, dbo.ModifierFlavour.price AS fprice, dbo.MenuItem.Name,dbo.MenuItem.Target, dbo.MenuItem.Id, dbo.MenuGroup.Id AS mid, dbo.Saledetails.Flavourid,                          dbo.ModifierFlavour.name AS Expr1, dbo.MenuGroup.Name AS Groupname FROM            dbo.Saledetails INNER JOIN                         dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id INNER JOIN                         dbo.Sale ON dbo.Saledetails.saleid = dbo.Sale.Id INNER JOIN                         dbo.MenuGroup ON dbo.MenuItem.MenuGroupId = dbo.MenuGroup.Id LEFT OUTER JOIN                         dbo.ModifierFlavour ON dbo.Saledetails.Flavourid = dbo.ModifierFlavour.Id WHERE    (dbo.Sale.date between '" + dateTimePicker3.Text + "' and '" + dateTimePicker4.Text + "') and  (dbo.Sale.BillStatus = 'Paid')  and dbo.Saledetails.ModifierId=0  and dbo.Saledetails.RunTimeModifierId=0   GROUP BY dbo.MenuItem.Name, dbo.ModifierFlavour.name, dbo.MenuItem.Id, dbo.MenuGroup.Id, dbo.Saledetails.Flavourid, dbo.MenuGroup.Name, dbo.MenuItem.Price, dbo.ModifierFlavour.price,dbo.MenuItem.Target";
                    DataSet dscost = objCore.funGetDataSet(q);
                    for (int k = 0; k < dscost.Tables[0].Rows.Count; k++)
                    {
                    ...
                    dscost = new DataSet();
                    q = "SELECT        TOP (100) PERCENT SUM(dbo.Saledetails.Price - dbo.Saledetails.Itemdiscount) AS price, SUM(dbo.Saledetails.Quantity) AS qty, dbo.Saledetails.ModifierId, dbo.Modifier.Name, dbo.Modifier.Price AS Expr1 FROM            dbo.Saledetails INNER JOIN                         dbo.Sale ON dbo.Saledetails.saleid = dbo.Sale.Id INNER JOIN                         dbo.Modifier ON dbo.Saledetails.ModifierId = dbo.Modifier.Id WHERE        (dbo.Sale.date between '" + dateTimePicker3.Text + "' and '" + dateTimePicker4.Text + "') AND (dbo.Sale.BillStatus = 'Paid') AND (dbo.Saledetails.ModifierId > 0)  AND (dbo.Saledetails.Price > 0)  GROUP BY dbo.Saledetails.ModifierId, dbo.Modifier.Name, dbo.Modifier.Price ORDER BY dbo.Modifier.Name";
                    dscost = objCore.funGetDataSet(q);
                    for (int k = 0; k < dscost.Tables[0].Rows.Count; k++)
                    {
                        cost = cost + getcostmenu(dscost.Tables[0].Rows[k]["ModifierId"].ToString(), "0", "modifier", "", "", "", "no");
                ...
                for (int j = 0; j < dsemp.Tables[0].Rows.Count; j++)
                {
                    ...
                    q = "select Empid, (amount) AS amount    FROM         EmployeesSalary   where date between '" + dateTimePicker3.Text + "' and  '" + dateTimePicker4.Text + "'  and Empid='" + dsemp.Tables[0].Rows[j]["Empid"].ToString() + "' ";
                    ds = objCore.funGetDataSet(q);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                ...
                for (int j = 0; j < dsemp.Tables[0].Rows.Count; j++)
                {
                    q = "SELECT        Name, (Amount) AS Amount FROM            Expenses  where date between '" + dateTimePicker3.Text + "' and '" + dateTimePicker4.Text + "'  and Name='" + dsemp.Tables[0].Rows[j]["name"].ToString() + "'  ";
                    ds = objCore.funGetDataSet(q);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        double amount = 0;

            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                try
                {
                    ...
                    if (editmode.ToString() == "void")
                    {
                        ...
                        try
                        {
                            q = "select terminal from sale where id='" + saleid + "' and billstatus='Paid'";
                            DataSet dsterminal = new DataSet();
                            dsterminal = objCore.funGetDataSet(q);
                            if (dsterminal.Tables[0].Rows.Count > 0)
                            {
                                if (dsterminal.Tables[0].Rows[0][0].ToString() != System.Environment.MachineName)
                                {
                                    MessageBox.Show("You Can not Perform This Action When Bill is Closed from Other Terminal");
                                    return;
                                }
                            }
                        }
                    ...
                    {
                        DuplicaeBill ob = new DuplicaeBill(_frm1);
                        ob.id = ds.Tables[0].Rows[0]["id"].ToString();
                        ob.date = datee;
                        ob.Show();

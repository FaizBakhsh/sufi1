...
{
    public partial class CriticalInventoryClosingonly : Form
    {
        POSRestaurant.classes.Clsdbcon objcore = new classes.Clsdbcon();
        public CriticalInventoryClosingonly()
        {
            InitializeComponent();
        }
        ...
        public void getdata(string search)
        {
            try
            {
                ...
                string qq = "insert into log (Name, Time, Description,userid) values ('Closing','" + DateTime.Now + "','Closing Only Page Opened','" + userid + "')";
            ...
            string q = "";
            // "SELECT     dbo.RawItem.id,dbo.RawItem.ItemName, dbo.InventoryConsumed.RemainingQuantity FROM         dbo.RawItem INNER JOIN                      dbo.InventoryConsumed ON dbo.RawItem.Id = dbo.InventoryConsumed.RawItemId where dbo.InventoryConsumed.date='" + dateTimePicker1.Text + "'";
            ...
            for (int i = 0; i < ds1.Tables[0].Rows.Count; i++)
            {
                ...
                ds.Rows.Add(ds1.Tables[0].Rows[i]["id"].ToString(), ds1.Tables[0].Rows[i]["Itemname"].ToString(), closing, remarks, user);
            }
        ...
        private void dataGridView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 2)
            {
                try
                {
                    ...
                    savedataclosing(dataGridView1.Rows[e.RowIndex].Cells[0].Value.ToString(), dataGridView1.Rows[e.RowIndex].Cells[3].Value.ToString(),  closing.ToString());
                    //double discard=Math.Round(((opening + purchaes+tin) - (consumed + staff + complete+tout)) - remaining, 2);
                    //discard = discard * -1;
                    dataGridView1.Rows[e.RowIndex].Cells[4].Value = username;
        ...
        protected void savedataclosing(string itemid, string remarks,  string remaining)
        {
            chk1 = false;
            string userid = POSRestaurant.Properties.Settings.Default.UserId.ToString();
            {

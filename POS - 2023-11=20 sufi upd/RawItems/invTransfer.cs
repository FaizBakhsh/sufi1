    ...
    public partial class invTransfer : Form
    {
        ...
        protected void getdata()
        {
            ...
            for (int i = 0; i < ds1.Tables[0].Rows.Count; i++)
            {
                ...
                q = "SELECT     (TransferIn) AS tin,(TransferOut) AS tout,Remarks,price,status,invoiceno,Percentage FROM     InventoryTransfer where Date ='" + date + "' and ItemId='" + ds1.Tables[0].Rows[i]["id"].ToString() + "' and branchid='" + comboBox1.SelectedValue + "'  and sourcebranchid='" + comboBox2.SelectedValue + "'";
                ...
                ds.Rows.Add(ds1.Tables[0].Rows[i]["id"].ToString(), ds1.Tables[0].Rows[i]["Itemname"].ToString(), tin, Percentage, price, Math.Round(tin * price, 2), remarks, status, ds1.Tables[0].Rows[i]["minorder"].ToString(), ds1.Tables[0].Rows[i]["maxorder"].ToString(), invoiceno);
            }
        ...
        private void dataGridView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                if (e.ColumnIndex == 2 || e.ColumnIndex == 3)
                {
                    ...
                    double Percentage = Convert.ToDouble(dataGridView1.Rows[e.RowIndex].Cells["Percentage"].Value.ToString()) / 100;
        ...
        private void vButton2_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow dr in dataGridView1.Rows)
            {
                ...
                barcode = "*" + barcode + "*";
                {
                    ...
                    {
                        q = "update InventoryTransfer set DemandDate='" + demanddate + "',barcode='" + barcode + "',status='Pending', uploadstatus='Pending',InvoiceNo='" + dr.Cells["InvoiceNo"].Value.ToString().Replace("&", "n") + "',Remarks='" + dr.Cells["Remarks"].Value.ToString().Replace("&", "n").Replace("'", "-") + "',TransferIn='" + Math.Round(Convert.ToDouble(dr.Cells["Transfer In"].Value.ToString()), 2).ToString() + "',Percentage ='" + Math.Round(Convert.ToDouble(dr.Cells["Percentage"].Value.ToString()), 2).ToString() + "',price ='" + Math.Round(Convert.ToDouble(dr.Cells["price"].Value.ToString()), 2).ToString() + "' ,total ='" + Math.Round(Convert.ToDouble(dr.Cells["total"].Value.ToString()), 2).ToString() + "'  where   itemid='" + dr.Cells["id"].Value.ToString() + "' and date='" + dateTimePicker1.Text + "' and branchid='" + comboBox1.SelectedValue + "' and sourcebranchid='" + comboBox2.SelectedValue + "'";
                    ...
                    else
                    {
                        ...
                        {
                            q = "insert into InventoryTransfer (DemandDate,barcode,InvoiceNo,status,Remarks,Date, Itemid, TransferIn, price, branchid,sourcebranchid,total,Percentage) values('" + demanddate + "','" + barcode + "','" + dr.Cells["InvoiceNo"].Value.ToString().Replace("&", "n") + "','Pending','" + dr.Cells["Remarks"].Value.ToString().Replace("&", "n").Replace("'", "-") + "','" + dateTimePicker1.Text + "','" + dr.Cells["id"].Value.ToString() + "','" + Math.Round(Convert.ToDouble(dr.Cells["Transfer In"].Value.ToString()), 2).ToString() + "','" + Math.Round(Convert.ToDouble(dr.Cells["price"].Value.ToString()), 2).ToString() + "','" + comboBox1.SelectedValue + "','" + comboBox2.SelectedValue + "','" + Math.Round(Convert.ToDouble(dr.Cells["total"].Value.ToString()), 2).ToString() + "','" + Math.Round(Convert.ToDouble(dr.Cells["Percentage"].Value.ToString()), 2).ToString() + "')";
                            objcore.executeQuery(q);
                            chk = true;
                        }
                    }

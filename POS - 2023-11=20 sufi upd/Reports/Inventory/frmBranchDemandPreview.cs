    ...
    public partial class frmBranchDemandPreview : Form
    {
        ...
        public void bindreport()
        {
            try
            {
                ...
                POSRestaurant.Reports.Inventory.rptBranchWiseDemad rptDoc1 = new Reports.Inventory.rptBranchWiseDemad();
                ...
                if (dt.Rows.Count > 0)
                {
                    if (branchid == "-1")
                    {
                        ...
                        rptDoc1.SetParameterValue("name", company);
                        rptDoc1.SetParameterValue("restaurant", "Restaurant: " + restaurant);

                        date = Convert.ToDateTime(date).ToString("dd-MM-yyyy");
                        rptDoc1.SetParameterValue("date", "Date:" + date);
        ...
        public DataTable getAllOrders()
        {
            ...
            try
            {
                ...
                if (branchid == "-1")
                {
                    q = "SELECT        dbo.RawItem.ItemName, dbo.UOM.UOM, sum(dbo.Demand.Quantity) Quantity, sum(dbo.Demand.Closing)as Closing, dbo.Demand.Itemid, dbo.Category.CategoryName FROM            dbo.Demand INNER JOIN                         dbo.RawItem ON dbo.Demand.Itemid = dbo.RawItem.Id INNER JOIN                         dbo.UOM ON dbo.RawItem.UOMId = dbo.UOM.Id INNER JOIN                         dbo.Category ON dbo.RawItem.CategoryId = dbo.Category.Id inner join branch b on b.id=Demand.BranchID WHERE        dbo.Demand.Date>='" + date + "'  group by  dbo.RawItem.ItemName,dbo.UOM.UOM,dbo.Demand.Itemid,dbo.Category.CategoryName   ";

                }
                else
                {
                    q = "SELECT        dbo.RawItem.ItemName, dbo.UOM.UOM, dbo.Demand.Quantity, dbo.Demand.Closing, dbo.Demand.Itemid, dbo.Category.CategoryName FROM            dbo.Demand INNER JOIN                         dbo.RawItem ON dbo.Demand.Itemid = dbo.RawItem.Id INNER JOIN                         dbo.UOM ON dbo.RawItem.UOMId = dbo.UOM.Id INNER JOIN                         dbo.Category ON dbo.RawItem.CategoryId = dbo.Category.Id inner join branch b on b.id=Demand.BranchID WHERE        dbo.Demand.Date='" + date + "' and dbo.Demand.branchid='" + branchid + "'";
                }

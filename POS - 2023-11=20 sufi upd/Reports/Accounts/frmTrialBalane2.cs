    ...
    public partial class frmTrialBalane2 : Form
    {
       ...
       public static string max = "";// Convert.ToDateTime(dateTimePicker2.Text).ToShortDateString();
        ...
        public void bindreport()
        {
            ...
            try
            {
                //min = Convert.ToDateTime(dateTimePicker1.Text).ToShortDateString();
                //max = Convert.ToDateTime(dateTimePicker2.Text).ToShortDateString();
        ...
        public DataTable getAllOrders()
        {
            ...
            for (int i = 0; i < dsaccounts.Tables[0].Rows.Count; i++)
            {
                ...
                getAllOrders1(dsaccounts.Tables[0].Rows[i]["id"].ToString(), cmbbranchjv.SelectedValue.ToString(), dsaccounts.Tables[0].Rows[i]["AccountCode"].ToString(), dsaccounts.Tables[0].Rows[i]["Name"].ToString(), dsaccounts.Tables[0].Rows[i]["AccountType"].ToString(), dsaccounts.Tables[0].Rows[i]["SubAccount"].ToString());
            }
            getcompany();
            string company = "", phone = "", address = "", logo = "";
            try
        ...
        string mtitle = "";
        string mcode = "";
        public void getAllOrders1(string id,string brid,string cd, string titl,string accounttype,string subaccount)
        {           
            try
            {
                string head = "";
                    ...
                    else
                    {
                        ...
                        //if (openingblnce >= 0)
                        {
                           // openingblnce = Math.Abs(openingblnce);
                            if (logo == "")
                            {
                                dtrpt.Rows.Add(head, accounttype, subaccount, titl, openingblnce, 0, debit, credit, 0, blnce, null, cmbbranchjv.Text);
                            }
                            else
                            {
                                dtrpt.Rows.Add(head, accounttype, subaccount, titl, openingblnce, 0, debit, credit, 0, blnce, dscompany.Tables[0].Rows[0]["logo"], cmbbranchjv.Text);
                            }
                        }

...
namespace POSRestaurant.Reports
{
    public partial class frmSales : Form
    {
        ...
        public DataTable getAllOrders()
        {
            ...
            double gross = 0, cashgst = 0, posfee = 0, complimtry = 0, cardgst = 0, gst = 0, cashdis = 0, carddis = 0, discount = 0, net = 0, service = 0, cashrecv = 0, cardrecv = 0, recv = 0, cash = 0, credit = 0, master = 0, dinin = 0, takeaway = 0, delivery = 0, refund = 0, voidsale = 0, carhope = 0, calculatedcash = 0, drawerfloat = 0, bankingtotal = 0, declared = 0, over = 0, total = 0;
                        ...
                        else
                        {
                            if (cmbshift.Text == "All")
                            {
                                if (cmbbranch.Text == "All")
                                {
                                    if (comboBox1.Text == "All Terminals")
                                    {
                                        q = "SELECT        SUM(dbo.Sale.TotalBill) AS gross, SUM(dbo.Sale.GST) AS gst, SUM(dbo.Sale.TotalBill) AS netsale, SUM(dbo.Sale.servicecharges) AS serv, SUM(dbo.Sale.DiscountAmount) AS discount, SUM(POSFee) AS POSFee FROM            dbo.Sale INNER JOIN                         dbo.DinInTables ON dbo.Sale.Id = dbo.DinInTables.Saleid where dbo.DinInTables.WaiterId='" + cmbservers.SelectedValue + "'  and (Date between '" + dateTimePicker1.Text + "' and '" + dateTimePicker2.Text + "')  and billstatus='Paid'";
                        ...
                        if (cmbservers.Text == "All")
                        {
                            if (cmbshift.Text == "All")
                            {
                                if (cmbbranch.Text == "All")
                                {
                                    if (comboBox1.Text == "All Terminals")
                                    {
                                        q = "SELECT     SUM(TotalBill) AS gross, SUM(GST) AS gst, SUM(TotalBill) AS netsale,sum(servicecharges) as serv, SUM(DiscountAmount) AS discount, SUM(POSFee) AS POSFee FROM         Sale where UserId='" + cmbcashier.SelectedValue + "' and  (Date between '" + dateTimePicker1.Text + "' and '" + dateTimePicker2.Text + "')  and billstatus='Paid'";

                                    }
                                    else
                                    {
                                        q = "SELECT     SUM(TotalBill) AS gross, SUM(GST) AS gst, SUM(TotalBill) AS netsale,sum(servicecharges) as serv, SUM(DiscountAmount) AS discount, SUM(POSFee) AS POSFee FROM         Sale where  UserId='" + cmbcashier.SelectedValue + "' and   (Date between '" + dateTimePicker1.Text + "' and '" + dateTimePicker2.Text + "') and terminal='" + comboBox1.Text + "'  and billstatus='Paid'";
            ...
            if (logo == "")
            {
                ...
                dat.Rows.Add(gross, gst, discount, net, cash, credit, master, dinin, takeaway, delivery, refund, voidsale, sht, username, Dorders, RefundNo, null, totlorder, avgsale, carhope, carhopeorders, calculatedcash, drawerfloat, bankingtotal, declared, over, total, service, recv, cashgst, cardgst, cashdis, carddis, cashrecv, cardrecv, avgdinein, avgdineintable, posfee, complimtry);

            }
            else
            {
                dat.Rows.Add(gross, gst, discount, net, cash, credit, master, dinin, takeaway, delivery, refund, voidsale, sht, username, Dorders, RefundNo, dscompany.Tables[0].Rows[0]["logo"], totlorder, avgsale, carhope, carhopeorders, calculatedcash, drawerfloat, bankingtotal, declared, over, total, service, recv, cashgst, cardgst, cashdis, carddis, cashrecv, cardrecv, avgdinein, avgdineintable, posfee, complimtry);
        ...
        private void vButton2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //MailAddress from = new MailAddress("admin@ftech.com.pk", "Ftech");
                //MailAddress tto = new MailAddress("younas0313@gmail.com");

                //MailMessage msg = new MailMessage(from, tto);

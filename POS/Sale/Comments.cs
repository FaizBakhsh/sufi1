        ...
        public void callgrid(vButton btn)
        {
            try
            {
                if (btn.Text != string.Empty)
                {
                    ...
                    dscallgrid = objCore.funGetDataSet("SELECT     dbo.Modifier.Id, dbo.Modifier.Name AS ModifierName, dbo.Modifier.Price, dbo.RawItem.ItemName AS name FROM         dbo.Modifier INNER JOIN                     dbo.RawItem ON dbo.Modifier.RawItemId = dbo.RawItem.Id where dbo.Modifier.Name='" + btn.Text + "' and dbo.Modifier.Menuitemid='" + id + "'");

...
namespace POSRestaurant.Sale
{
    public partial class NewBillTypenew : Form
    {
        ...
        public void getsaleid(string tbno)
        {
            ...
            string TableNo = "";
            string Guset = txtguest.Text;

            if (name.Length < 2)
            {
                string CusName = name;
                TableNo = name;
                if (DinMobile.Length > 2)
                {
                    name = Guset + " " + DinInCustomer + " Mobile # " + DinMobile;
                }
                else
                {

                    name = Guset + " " + DinInCustomer;
            ...
            q = "insert into DinInTables (Guests,id,TableNo,Saleid,Date,time,Status,WaiterId) values('" + name + "','" + id + "','" + tbno + "','" + saleid + "','" + date + "','" + DateTime.Now.ToShortTimeString() + "','Pending','" + comboBox1.SelectedValue + "')";
        ...
        private void vButton46_Click(object sender, EventArgs e)
        {
            ...
            if (rename == "yes")
            {
                try
                {
                    string q = "update sale set ordertype='Dine In',customer='" + name + "' where id='" + saleid + "'";
                    objCore.executeQuery(q);
                    q = "update DinInTables set TableNo='" + TableNo + "',WaiterId='" + comboBox1.SelectedValue + "',Guests='" + txtguest.Text + "' where saleid='" + saleid + "'";
                    objCore.executeQuery(q);
                    _frm1.getorders("new");
                    _frm1.Enabled = true;
                    this.Close();
        ...
        private void tblfloor_Paint(object sender, PaintEventArgs e)
        {

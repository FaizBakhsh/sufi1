...
{
    public partial class kdsnewfix : Form
    {
        public kdsnewfix()
        {
            InitializeComponent();
        ...
        string getkdsid(string table,string id)
        {
            string kid = "";
            try
            {
                string q = "select kdsid from " + table + " where id=" + id;
                DataSet dsk = new DataSet();
                dsk = objCore.funGetDataSet(q);
                if (dsk.Tables[0].Rows.Count > 0)
                {
                    kid = dsk.Tables[0].Rows[0][0].ToString();
                }
            }
            catch (Exception ex)
            {
        ...
        {
            string q = "SELECT dbo.Saledetails.Id,dbo.Saledetails.menuitemid, dbo.Sale.Terminal,dbo.MenuItem.kdsId,dbo.Saledetails.RunTimeModifierId,dbo.Saledetails.ModifierId,dbo.Saledetails.Flavourid FROM  dbo.Saledetails INNER JOIN               dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id INNER JOIN               dbo.Sale ON dbo.Saledetails.saleid = dbo.Sale.Id WHERE (dbo.Saledetails.saleid = '" + id + "') and (dbo.Saledetails.kdsgroup = '" + group + "') ";
            ...
            for (int i = 0; i < dsd.Tables[0].Rows.Count; i++)
            {
                if (terminal == "")
                {
                    if (dsd.Tables[0].Rows[i]["RunTimeModifierId"].ToString().ToLower() != "0")
                    {
                        kid = getkdsid("RuntimeModifier", dsd.Tables[0].Rows[i]["RunTimeModifierId"].ToString().ToLower());
                    }
                    else if (dsd.Tables[0].Rows[i]["ModifierId"].ToString().ToLower() != "0")
                    {
                        kid = getkdsid("Modifier", dsd.Tables[0].Rows[i]["ModifierId"].ToString().ToLower());
                    }
                    else
                    {
                        kid = getkdsid("MenuItem", dsd.Tables[0].Rows[i]["menuitemid"].ToString().ToLower());
                    }
                    if (kdsid == kid || kid == "0")
                    {
                        //if (kid == "0")
                        //{
                        //    q = "SELECT        dbo.Attachmenu1.id, dbo.Attachmenu1.menuitemid, dbo.Attachmenu1.Flavourid, dbo.Attachmenu1.attachmenuid, dbo.Attachmenu1.attachFlavourid, dbo.Attachmenu1.Quantity, dbo.Attachmenu1.status, dbo.MenuItem.KDSId FROM            dbo.Attachmenu1 INNER JOIN                         dbo.MenuItem ON dbo.Attachmenu1.attachmenuid = dbo.MenuItem.Id where menuitemid='" + dsd.Tables[0].Rows[i]["MenuItemId"].ToString() + "'";
                        ...
                        //        {
                        //            //if (dsattach.Tables[0].Rows[l]["KDSId"].ToString() == kdsid) -- Yasir
                        //            if (dsattach.Tables[0].Rows[l]["KDSId"].ToString() == kdsid)
                        //            {
                                         q = "insert into Ordercomplete (saledetailsid, itemid) values('" + dsd.Tables[0].Rows[i]["id"].ToString() + "','" + dsd.Tables[0].Rows[i]["menuitemid"].ToString() + "')";
                        ...
                        //else
                        //{
                        q = "update Saledetails set OrderStatus='Complete',completedtime='" + DateTime.Now + "' where id='" + dsd.Tables[0].Rows[i]["id"] + "'";
                            objCore.executeQuery(q);
                        //}
                    }
                }
                else
                {
                    if (terminal.ToLower() == dsd.Tables[0].Rows[i]["Terminal"].ToString().ToLower())
                    {
                        ...
                        {
                            q = "update Saledetails set OrderStatus='Completed',OrderStatusmain='Completed',completedtime='" + DateTime.Now + "' where id='" + dsd.Tables[0].Rows[i]["id"] + "'";
                            objCore.executeQuery(q);
                        ...
                        {
                            q = "update Saledetails set OrderStatusmain='Completed',completedtime='" + DateTime.Now + "' where id='" + dsd.Tables[0].Rows[i]["id"] + "'";
                            objCore.executeQuery(q);
                            q = "update sale set DeliveredTime='" + DateTime.Now + "' where id='" + id + "'";
                            objCore.executeQuery(q);
                        }
                    ...
                    else
                    {
                        if (kdsid == dsd.Tables[0].Rows[i]["kdsid"].ToString().ToLower())
                        {
                            q = "update Saledetails set OrderStatus='Completed',completedtime='" + DateTime.Now + "' where id='" + dsd.Tables[0].Rows[i]["id"] + "'";
                            objCore.executeQuery(q);
        ...
        public void fillpending(string refresh)
        {
            try
            {
                ...
                dtds.Columns.Add("id", typeof(string));
                dtds.Columns.Add("Customer", typeof(string));
                dtds.Columns.Add("time", typeof(string));
                dtds.Columns.Add("terminal", typeof(string));
                dtds.Columns.Add("TerminalOrder", typeof(string));
                dtds.Columns.Add("kdsgroup", typeof(string));
                ...
                if (terminal.Trim() != "")
                {
                   ...
                   //q = "SELECT DISTINCT TOP " + top + "  dbo.Sale.id as id, dbo.Sale.Customer, dbo.Sale.time, dbo.Sale.terminal, dbo.Sale.TerminalOrder, dbo.Saledetails.kdsgroup FROM  dbo.Saledetails INNER JOIN               dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id INNER JOIN              dbo.Sale ON dbo.Saledetails.saleid = dbo.Sale.Id WHERE ((dbo.Saledetails.OrderStatusmain = 'Pending') and dbo.Sale.Terminal='" + terminal + "'  AND (dbo.Saledetails.kdsgroup > 0) ) or ((dbo.Saledetails.Orderstatus = 'Pending') and dbo.Saledetails.KDSId = '" + kdsid + "'  AND (dbo.Saledetails.kdsgroup > 0) ) ORDER BY dbo.Saledetails.kdsgroup";

                   //q = "SELECT DISTINCT TOP " + top + "  dbo.Sale.id as id, dbo.Sale.Customer, dbo.Sale.time, dbo.Sale.terminal, dbo.Sale.TerminalOrder, dbo.Saledetails.kdsgroup FROM  dbo.Saledetails INNER JOIN               dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id INNER JOIN              dbo.Sale ON dbo.Saledetails.saleid = dbo.Sale.Id WHERE  ((dbo.Saledetails.Orderstatus = 'Pending') and dbo.Sale.Terminal='" + terminal + "'   and dbo.Saledetails.KDSId = '" + kdsid + "'  AND (dbo.Saledetails.kdsgroup > 0) )  or  dbo.Sale.Terminal='" + terminal + "'   and (dbo.Saledetails.Orderstatus = 'Pending') AND (dbo.Saledetails.KDSId = '0') AND (dbo.Saledetails.kdsgroup > 0)    ORDER BY dbo.Saledetails.kdsgroup";
                }
                else
                {
                  //  q = "SELECT DISTINCT TOP " + top + "  dbo.Saledetails.saleid as id, dbo.Sale.Customer, dbo.Sale.time, dbo.Sale.terminal, dbo.Sale.TerminalOrder, dbo.Saledetails.kdsgroup FROM  dbo.Saledetails INNER JOIN               dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id INNER JOIN              dbo.Sale ON dbo.Saledetails.saleid = dbo.Sale.Id WHERE (dbo.Saledetails.Orderstatus = 'Pending') AND (dbo.Saledetails.KDSId = '" + kdsid + "') AND (dbo.Saledetails.kdsgroup > 0)  or (dbo.Saledetails.Orderstatusmain = 'Pending') AND (dbo.Saledetails.KDSId = '0') AND (dbo.Saledetails.kdsgroup > 0)    ORDER BY dbo.Saledetails.kdsgroup";
                    q = "SELECT DISTINCT TOP " + top + "  dbo.Saledetails.saleid as id, dbo.Sale.Customer, dbo.Sale.time, dbo.Sale.terminal, dbo.Sale.TerminalOrder, dbo.Saledetails.kdsgroup FROM  dbo.Saledetails INNER JOIN               dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id INNER JOIN              dbo.Sale ON dbo.Saledetails.saleid = dbo.Sale.Id WHERE (dbo.Saledetails.Orderstatus = 'Pending') AND (dbo.Saledetails.KDSId = '" + kdsid + "') AND (dbo.Saledetails.kdsgroup > 0) or (dbo.Saledetails.Orderstatus = 'Pending') AND (dbo.Saledetails.KDSId = '0') AND (dbo.Saledetails.kdsgroup > 0)      ORDER BY dbo.Saledetails.kdsgroup";
                ...
                for (int i = 0; i < dspr.Tables[0].Rows.Count; i++)
                {
                    ...
                    q = "select * from saledetails where orderstatus='Pending'  and saleid=" + saleid;
                    DataSet dsd = new DataSet();
                    dsd = objCore.funGetDataSet(q);
                    if (dsd.Tables[0].Rows.Count > 0)
                    {
                        ...
                        {
                            if (dsd.Tables[0].Rows[j]["ModifierId"].ToString() == "0" && dsd.Tables[0].Rows[j]["RunTimeModifierId"].ToString() == "0" )
                            {
                                try
                                {
                                    if (dsd.Tables[0].Rows[j]["kdsid"].ToString()=="0")
                                    {
                                        q = "select id from Attachmenu1 where menuitemid=" + dsd.Tables[0].Rows[j]["MenuItemId"].ToString();
                                        DataSet dschk = new DataSet();
                                        dschk = objCore.funGetDataSet(q);
                                        if (dschk.Tables[0].Rows.Count > 0)
                                        {
                                            q = "SELECT        dbo.Attachmenu1.id, dbo.Attachmenu1.menuitemid, dbo.Attachmenu1.Flavourid, dbo.Attachmenu1.attachmenuid, dbo.Attachmenu1.attachFlavourid, dbo.Attachmenu1.Quantity, dbo.Attachmenu1.status, dbo.MenuItem.KDSId FROM            dbo.Attachmenu1 INNER JOIN                         dbo.MenuItem ON dbo.Attachmenu1.attachmenuid = dbo.MenuItem.Id where menuitemid='" + dsd.Tables[0].Rows[j]["MenuItemId"].ToString() + "' and dbo.MenuItem.KDSId='" + kdsid + "'";
                                            DataSet dsattach = new DataSet();
                                            dsattach = objCore.funGetDataSet(q);
                                            if (dsattach.Tables[0].Rows.Count > 0)
                                            {
                                                for (int l = 0; l < dsattach.Tables[0].Rows.Count; l++)
                                                {
                                                    q = "SELECT        dbo.Ordercomplete.id, dbo.Ordercomplete.saledetailsid, dbo.Ordercomplete.itemid, dbo.MenuItem.KDSId FROM            dbo.Ordercomplete INNER JOIN                         dbo.MenuItem ON dbo.Ordercomplete.itemid = dbo.MenuItem.Id where saledetailsid='" + dsd.Tables[0].Rows[j]["id"].ToString() + "' and itemid='" + dsattach.Tables[0].Rows[l]["attachmenuid"].ToString() + "' and dbo.MenuItem.KDSId='" + kdsid + "'";
                                                    DataSet dsorder = new DataSet();
                                                    dsorder = objCore.funGetDataSet(q);
                            ...
                            {
                                if (dsd.Tables[0].Rows[j]["kdsid"].ToString() == kdsid)
                                {
                                    chk = false;
                    ...
                    if (chk == false)
                    {
                        dtds.Rows.Add(saleid, dspr.Tables[0].Rows[i]["customer"].ToString(), dspr.Tables[0].Rows[i]["time"].ToString(), dspr.Tables[0].Rows[i]["terminal"].ToString(), dspr.Tables[0].Rows[i]["TerminalOrder"].ToString(), dspr.Tables[0].Rows[i]["kdsgroup"].ToString());
                    }
                }
                ds = new DataSet();
                ds.Tables.Add(dtds);
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (i == 0)
                    {
                        ...
                        btn.Text = addon + dt.Rows[0][8].ToString() + "-Order No:" + ds.Tables[0].Rows[i]["id"].ToString() + " ! " + ds.Tables[0].Rows[i]["Customer"].ToString() +"-"+ ds.Tables[0].Rows[i]["TerminalOrder"].ToString() + "> " + timediff;
                        try
                        {
                    ...
                    if (i == 1)
                    {
                        ...
                        btn.Text = addon + dt1.Rows[0][8].ToString() + "-Order No:" + ds.Tables[0].Rows[i]["id"].ToString() + " ! " + ds.Tables[0].Rows[i]["Customer"].ToString() + "-" + ds.Tables[0].Rows[i]["TerminalOrder"].ToString() + "> " + timediff;
                        try
                        {
                            if (ds.Tables[0].Rows[i]["terminal"].ToString().ToLower().Contains("dt"))
                            {
        ...
        public DataTable getdata(string id,string group)
        {
            ...
            try
            {
                ...
                if (terminal.Trim() == "")
                {
                    q = "SELECT     dbo.Saledetails.Orderstatus,dbo.sale.Terminal,dbo.Sale.id,dbo.Saledetails.time,dbo.MenuItem.Name,dbo.MenuItem.Minutes,dbo.MenuItem.alarmtime,dbo.MenuItem.minuteskdscolor,dbo.MenuItem.alarmkdscolor, dbo.Saledetails.id as sid,dbo.Saledetails.menuitemid,dbo.Saledetails.Flavourid, dbo.Saledetails.ModifierId,dbo.Saledetails.RunTimeModifierId, dbo.Saledetails.Quantity,dbo.Saledetails.comments, dbo.Saledetails.KDSId, dbo.Sale.OrderType FROM         dbo.Sale INNER JOIN                      dbo.Saledetails ON dbo.Sale.Id = dbo.Saledetails.saleid INNER JOIN                      dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id where  dbo.Sale.id='" + id + "' and  (dbo.Saledetails.Orderstatus = 'Pending') and dbo.Saledetails.kdsgroup ='" + group + "'  and dbo.Saledetails.KDSId='0'  order by dbo.Saledetails.id";
            ...
            {
                q = "SELECT     dbo.Saledetails.Orderstatus,dbo.sale.Terminal,dbo.Sale.id,dbo.Saledetails.time,dbo.MenuItem.Name,dbo.MenuItem.Minutes,dbo.MenuItem.alarmtime,dbo.MenuItem.minuteskdscolor,dbo.MenuItem.alarmkdscolor, dbo.Saledetails.id as sid,dbo.Saledetails.menuitemid,dbo.Saledetails.Flavourid, dbo.Saledetails.ModifierId,dbo.Saledetails.RunTimeModifierId, dbo.Saledetails.Quantity,dbo.Saledetails.comments, dbo.MenuItem.KDSId, dbo.Sale.OrderType FROM         dbo.Sale INNER JOIN                      dbo.Saledetails ON dbo.Sale.Id = dbo.Saledetails.saleid INNER JOIN                      dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id where  dbo.Sale.id='" + id + "' and  (dbo.Saledetails.Orderstatus = 'Pending') and dbo.Saledetails.kdsgroup ='" + group + "'  and dbo.Saledetails.KDSId='" + kdsid + "'  order by dbo.Saledetails.id";
            }
            ds = objCore.funGetDataSet(q);
            if (ds.Tables[0].Rows.Count > 0)
            {

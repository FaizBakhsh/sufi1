         ...
         public void getdata()
         {
             try
             {
                 ...
                 if (txtsearch.Text == string.Empty)
                 {
                     ...
                     q9 = "SELECT     dbo.Sale.Id AS Bill_No, dbo.Sale.Date, dbo.Sale.time, dbo.Sale.NetBill, dbo.Sale.BillType, dbo.Sale.TotalBill, dbo.Sale.DiscountAmount, dbo.Sale.GST, dbo.Sale.Customer, dbo.Sale.GSTPerc,                       dbo.Sale.Discount, dbo.Users.Name AS cashier, dbo.Sale.OrderType, dbo.Sale.servicecharges FROM         dbo.Sale INNER JOIN                      dbo.Users ON dbo.Sale.UserId = dbo.Users.Id where dbo.Sale.date='" + date + "' order by  dbo.Sale.id desc";
          
                 }
                 else
                 {

                     q9 = "SELECT     dbo.Sale.Id AS Bill_No, dbo.Sale.Date, dbo.Sale.time, dbo.Sale.NetBill, dbo.Sale.BillType, dbo.Sale.TotalBill, dbo.Sale.DiscountAmount, dbo.Sale.GST, dbo.Sale.Customer, dbo.Sale.GSTPerc,                       dbo.Sale.Discount, dbo.Users.Name AS cashier, dbo.Sale.OrderType, dbo.Sale.servicecharges FROM         dbo.Sale INNER JOIN                      dbo.Users ON dbo.Sale.UserId = dbo.Users.Id where dbo.Sale.id='" + txtsearch.Text.Trim() + "' dbo.Sale.date='" + date + "' order by  dbo.Sale.id desc";
                    ...
                    if (dr.Cells["Id"].Value.ToString() != string.Empty)
                    {
                        ...
                        if (rmid == "")
                        {
                            if (mid != string.Empty)
                            {
                                if (logo == "")
                                {
                                    ...
                                    dtrpt.Rows.Add(dr.Cells["Quantity"].Value.ToString(), dr.Cells["Modifier"].Value.ToString(), Convert.ToDouble(dr.Cells["Price"].Value.ToString()), Convert.ToDouble(totalbil), Convert.ToDouble(discount), Convert.ToDouble(GST), Convert.ToDouble(NetBill), user, cname, caddress, cphone, mp, siid, date, time, dr.Cells["OrderType"].Value.ToString(), Convert.ToDouble(amount), null);
                                }
                                else
                                {
                            ...
                            else
                            {
                                if (logo == "")
                                {
                                    ...
                                    dtrpt.Rows.Add((dr.Cells["Quantity"].Value.ToString()), dr.Cells["Item"].Value.ToString().Replace("Open Item", "") + " " + dr.Cells["Comments"].Value.ToString(), Convert.ToDouble(dr.Cells["Price"].Value.ToString()), Convert.ToDouble(totalbil), Convert.ToDouble(discount), Convert.ToDouble(GST), Convert.ToDouble(NetBill), user, cname, caddress, cphone, mp, siid, date, time, dr.Cells["OrderType"].Value.ToString(), Convert.ToDouble(amount), null);
                                }
                                else
                                {
                                    dtrpt.Rows.Add((dr.Cells["Quantity"].Value.ToString()), dr.Cells["Item"].Value.ToString().Replace("Open Item", "") + " " + dr.Cells["Comments"].Value.ToString(), Convert.ToDouble(dr.Cells["Price"].Value.ToString()), Convert.ToDouble(totalbil), Convert.ToDouble(discount), Convert.ToDouble(GST), Convert.ToDouble(NetBill), user, cname, caddress, cphone, mp, siid, date, time, dr.Cells["OrderType"].Value.ToString(), Convert.ToDouble(amount), dsinfo.Tables[0].Rows[0]["logo"]);
                                }
                            }
                        }
                        else
                        {
                            if (logo == "")
                            {
                                ...
                                dtrpt.Rows.Add(dr.Cells["Quantity"].Value.ToString(), dr.Cells["Rmodifier"].Value.ToString(), Convert.ToDouble(dr.Cells["Price"].Value.ToString()), Convert.ToDouble(totalbil), Convert.ToDouble(discount), Convert.ToDouble(GST), Convert.ToDouble(NetBill), user, cname, caddress, cphone, mp, siid, date, time, dr.Cells["OrderType"].Value.ToString(), Convert.ToDouble(amount), null);
                            }
                            else
                            {
            ...
            try
            {
                ...
                if (gridindex >= 0)
                {
                    ...
                    if (dr == DialogResult.Yes)
                    {
                        ...
                        string date = dataGridView1.Rows[gridindex].Cells[1].Value.ToString();
                        ...
                        else if (printtype().ToLower() == "rdlc")
                        {
                            ...
                            PrintClass.Printt(path, dt, billtype, saleid.ToString(), "", ordertype, totalbil, NetBill, discount, GSTperc, "0", "0", printername, info, print, discountamount.ToString(), GST.ToString(), customermsg, customermsg2, svschrgs, customer, date, "", _frm1, "", url, invoiceno,"Duplicate Bill");

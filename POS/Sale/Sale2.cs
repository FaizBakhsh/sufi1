...
namespace POSRestaurant.Sale
{
    public partial class Sale1 : Form
    {
        ...
        public void fillgrid(string id,string mid , string itmname,string price ,string q,string saletyp , string saledetailsid)
        {
            try
            {
                if (mid != string.Empty)
                {
                    try
                    {
                      //  obcustomerdisplay.fillgrid(id, mid, itmname, price.ToString(), q.ToString(),txtdiscount.Text,get);
                    }
                    catch (Exception ex)
                    {
        ...
        public void callfillgrid(Button name)
        {
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    try
                    {
                        //obcustomerdisplay.fillgrid(ds.Tables[0].Rows[0]["id"].ToString(), "", ds.Tables[0].Rows[0]["name"].ToString(), prc.ToString(), quantity.ToString(), txtdiscount.Text);
        ...
        private void button52_Click(object sender, EventArgs e)
        {
            ...
            //POSRestaurant.Sale.AllowDiscount ob = new AllowDiscount(this);
            //ob.Show();
        }
        ...
        private void vButton9_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //POSRestaurant.Sale.TableOrder ob = new TableOrder(this);

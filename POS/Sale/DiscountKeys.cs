...
namespace POSRestaurant.Sale
{
    public partial class DiscountKeys : Form
    {
        ...
        private void vButton2_Click(object sender, EventArgs e)
        {
            ...
            DataSet dschk = new DataSet();
            string q = "SELECT     id, name, discount,ISNULL(limit,'0') limit FROM         DiscountKeys where id='" + btn.Name + "'";
            dschk = objCore.funGetDataSet(q);
            if (dschk.Tables[0].Rows.Count > 0)
            {
                if (checkdis() == true)
                {
                    ...
                    limit = Convert.ToInt32(dschk.Tables[0].Rows[0]["limit"].ToString());
                    ob.userid = getuser();
                    ob.Show();
                    this.Close();
                }
                ...
                {
                    if (dschk.Tables[0].Rows[0]["limit"] != DBNull.Value && !string.IsNullOrEmpty(dschk.Tables[0].Rows[0]["limit"].ToString()))
                    {
                        limit = Convert.ToInt32(dschk.Tables[0].Rows[0]["limit"].ToString());
                    ...
                    _frm1.discountkeys(dschk.Tables[0].Rows[0]["discount"].ToString(), dschk.Tables[0].Rows[0]["name"].ToString(),limit.ToString(),txtDiscountNaration.Text);

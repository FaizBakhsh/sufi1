       ...
       public static void Printt(string path, DataTable dtprint, string mop, string sid, string cardno, string ordertype, string total, string nettotal, string discount, string gst, string cash, string change, string printername, string deliveryinfo, int prints, string discountamount, string gstamount, string msg1, string msg2, string serice,string cashier,string date,string delivery,RestSale _frm,string qrcode,string pointsurl,string invoiceno,string billtype)
       {
           ...
           try
           {
              ...
              /* try
               ...
               ReportParameter rbillno = new ReportParameter("billno", sid); // Your Bill No.
               ...
               try
               {
                   ...
                   if (DelvAmt == 0 && ordertype == "Delivery")
                   {
                       //DelvAmt = 100;
                       if (DelvAmt > 0)
                       {
                           ...
                           q = "update sale set  NetBill='" + net + "' , DeliveryAmt='" + DelvAmt + "'  where id=" + sid;
                          // q = "update sale set DeliveryAmt='" + DelvAmt + "'  where id=" + sid;
                       objcore.executeQuery(q);

                       }
                   
                   }
                   else if (DelvAmt > 0 && ordertype == "Delivery")
                   {
                       if (billtype != "Duplicate Bill")
                       {
                           dsDelv = new DataSet();
                   string s = "select deliveryAmt  from Sale where Id= " + sid;
                   dsDelv = objcore.funGetDataSet(s);
                   ...
                   if (dsDelv.Tables[0].Rows.Count > 0)
                   {
                       ...
                       else
                       {
                           ...
                           q = "update sale set  NetBill='" + nettotal + "' , DeliveryAmt='" + DelvAmt + "'  where id=" + sid;
                           // q = "update sale set DeliveryAmt='" + DelvAmt + "'  where id=" + sid;
                           objcore.executeQuery(q);
                       }
                   }
               ...
               else
               {
                   ...
                   else
                   {
                       try
                       {
                           ...
                           if (DelvAmt > 0)
                           {
                               net = titlet + Math.Round(Math.Round(Convert.ToDouble(gst1) + Convert.ToDouble(total) + Convert.ToDouble(DelvAmt) + Convert.ToDouble(servicechargescash) - Convert.ToDouble(discountamount), 2), 0);

                               netcard = titlet + Math.Round(Convert.ToDouble(gst2) + Convert.ToDouble(total) + Convert.ToDouble(DelvAmt)+ Convert.ToDouble(servicechargescard) - Convert.ToDouble(discountamount), 2) ;

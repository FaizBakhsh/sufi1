...
{
    public partial class ProductionPercentageWise : Form
    {
        POSRestaurant.classes.Clsdbcon objcore = new classes.Clsdbcon();
        public ProductionPercentageWise()
        {
            InitializeComponent();
        }
        public void getdata()
        {
            ...
            if(txtTotalQty.Text.Length>0){
                q = "SELECT RawItemID,RawItemPercentage FROM RawItemPercentageWise where RawItemPercentage>0";
                dsRawItem = objcore.funGetDataSet(q);
                TotalQty = Convert.ToDouble(txtTotalQty.Text);
            }
            for (int i = 0; i < ds1.Tables[0].Rows.Count; i++)
            {
                ...
                if (dsrecipe.Tables[0].Rows.Count > 0)
                {
                    ...
                    DataSet dsRawItemPwise = new DataSet();
                    string remarks = "";
                        ...
                        if (Id > 0)
                        {
                            if (dsRawItem.Tables[0].Rows.Count > 0)
                            {
                                var result =
                                    dsRawItem.Tables[0]
                                    .AsEnumerable()
                                    .Where(dr => dr.Field<int>("RawItemID") == Id).FirstOrDefault();
                                if (result != null)
                                {
                                    double Percentage = Convert.ToDouble(result.Field<double>("RawItemPercentage"));

                                    if (Percentage > 0)
                                    {
                                        double qty = (TotalQty * Percentage / 100);
                                        quantity = qty;
                                    }
                                }
                            }

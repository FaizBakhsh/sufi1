    ...
    public partial class DemandList2 : Form
    {
        ...
        private void PurChase_List_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                DataRow pleaseSelectRow = ds.Tables[0].NewRow();
                pleaseSelectRow["id"] = -1; // Set an appropriate ID or value for the "Please select" item
                pleaseSelectRow["branchName"] = "All";
                ds.Tables[0].Rows.InsertAt(pleaseSelectRow, 0);
        ...
        protected void getdata()
        {
            try
            {
                ...
                {
                    q = "SELECT       Date, Status, COUNT(*) AS TotalQuantity FROM            dbo.Demand where date between '" + dateTimePicker1.Text + "' and '" + dateTimePicker2.Text + "' GROUP BY  Date, Status order by date desc";

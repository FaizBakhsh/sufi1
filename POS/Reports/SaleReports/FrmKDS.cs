        ...
        public DataTable getAllOrders()
        {
            ...
            try
            {
                ...
                string q = "";
                    ...
                    if (textBox1.Text == "")
                    {
                        q = "SELECT        dbo.ModifierFlavour.name AS size, dbo.MenuItem.Name, dbo.Modifier.Name AS Modifier, dbo.RuntimeModifier.name AS RModifier, dbo.Saledetails.Quantity, dbo.Saledetails.time as OrderTime, dbo.Saledetails.completedtime,DATEDIFF(MINUTE,dbo.Saledetails.time , dbo.Saledetails.completedtime) as Minutes,Sale.Id FROM            dbo.Saledetails INNER JOIN                         dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id INNER JOIN                         dbo.Sale ON dbo.Saledetails.saleid = dbo.Sale.Id LEFT OUTER JOIN                         dbo.ModifierFlavour ON dbo.Saledetails.Flavourid = dbo.ModifierFlavour.Id LEFT OUTER JOIN                         dbo.RuntimeModifier ON dbo.Saledetails.RunTimeModifierId = dbo.RuntimeModifier.id LEFT OUTER JOIN                         dbo.Modifier ON dbo.Saledetails.ModifierId = dbo.Modifier.Id where dbo.sale.date between '" + dateTimePicker1.Text + "' and '" + dateTimePicker2.Text + "' order by dbo.menuitem.name";
                    }
                    else
                    {

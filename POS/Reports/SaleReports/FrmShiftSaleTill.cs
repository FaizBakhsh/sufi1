...
{
    public partial class FrmShiftSaleTill : Form
    {
        public string date = "", userid = "", cashiername = "";
        POSRestaurant.classes.Clsdbcon objCore = new classes.Clsdbcon();
        public FrmShiftSaleTill()
        {
            InitializeComponent();
        ...
        string delievrysourcetitle = "", delievrysourcedata = "", takawayorders = "",  deliveryorders = "";
        protected void deliverysource()
        {
            ...
            try
            {
                q = "SELECT        SUM(dbo.Sale.NetBill) AS amount, dbo.Delivery.type FROM            dbo.Sale INNER JOIN                          dbo.Delivery ON dbo.Sale.Id = dbo.Delivery.SaleId where  (dbo.sale.Date between '" + dateTimePicker2.Text + "' and '" + dateTimePicker2.Text + "')  and  dbo.Sale.Terminal='" + cmbterminal.Text.ToString() + "' and dbo.sale.shiftid='" + comboBox1.SelectedValue + "' and billstatus='Paid'  GROUP BY dbo.Delivery.type ";
        ...
        public DataTable getAllOrders()
        {
            ...
            double gross = 0, cashgst = 0,posfee=0, visagst = 0, gst = 0, cashdis = 0, carddis = 0, discount = 0, net = 0, cash = 0, credit = 0, master = 0, dinin = 0, recvble = 0, service = 0, takeaway = 0, delivery = 0, refund = 0, voidsale = 0, carhope = 0, calculatedcash = 0, drawerfloat = 0, bankingtotal = 0, declared = 0, over = 0, total = 0;
            string Dlorders = "0", Torders = "0", Dorders = "0", RefundNo = "0", carhopeorders = "0";
            double avgdinein = 0, avgdineintable = 0, cashrecv = 0, cardrecv=0;
            DataSet ds = new DataSet();
            ...
            try
            {
                ...
                {
                    ds = new DataSet();
                    q = "SELECT     SUM(TotalBill) AS gross, SUM(GST) AS gst, SUM(servicecharges) AS service, SUM(TotalBill) AS netsale, SUM(DiscountAmount) AS discount, SUM(POSFee) AS POSFee FROM         Sale where  (Date = '" + dateTimePicker2.Text + "') and dbo.Sale.shiftid='" + comboBox1.SelectedValue + "' and  dbo.Sale.Terminal='" + cmbterminal.Text.ToString() + "'  and dbo.Sale.billstatus='Paid'";
                    ds = objCore.funGetDataSet(q);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        //try
                        //{
                        //    string q1 = "SELECT        SUM(dbo.DiscountIndividual.Discount) AS Expr1 FROM            dbo.Sale INNER JOIN                         dbo.DiscountIndividual ON dbo.Sale.Id = dbo.DiscountIndividual.Saleid  where   (dbo.Sale.Date = '" + dateTimePicker2.Text + "') and dbo.Sale.shiftid='" + comboBox1.SelectedValue + "' and  dbo.Sale.Terminal='" + cmbterminal.Text.ToString() + "'  and dbo.Sale.billstatus='Paid'";
                       ...
                       // }
                        ...
                        {
                            ds = new DataSet();
                            q = "SELECT     SUM(DeliveryAmt) AS cash , count(id) as count FROM         Sale where  (Date = '" + dateTimePicker2.Text + "') and shiftid='" + comboBox1.SelectedValue + "'and Terminal='" + cmbterminal.Text.ToString() + "' and billstatus='Paid' and OrderType='Delivery'";
                            ds = objCore.funGetDataSet(q);
                            if (ds.Tables[0].Rows.Count > 0)
                            {
                        ...
                        gross = gst + service + gross + posfee;
                        net = Math.Round(net, 2);
                        try
                        {
                            q = "SELECT     SUM(TotalBill) AS gross, SUM(GST) AS gst,SUM(servicecharges) AS service, SUM(TotalBill) AS netsale, SUM(DiscountAmount) AS discount FROM         Sale where  (Date = '" + dateTimePicker2.Text + "') and dbo.Sale.shiftid='" + comboBox1.SelectedValue + "' and  dbo.Sale.Terminal='" + cmbterminal.Text.ToString() + "'   and billstatus='Paid' and  GSTtype='Cash'";
                            ...
                            q = "SELECT     SUM(TotalBill) AS gross, SUM(GST) AS gst,SUM(servicecharges) AS service, SUM(TotalBill) AS netsale, SUM(DiscountAmount) AS discount FROM         Sale where  (Date = '" + dateTimePicker2.Text + "') and dbo.Sale.shiftid='" + comboBox1.SelectedValue + "' and  dbo.Sale.Terminal='" + cmbterminal.Text.ToString() + "'   and billstatus='Paid' and  GSTtype='Card'";
                            dscash = new DataSet();
                            dscash = objCore.funGetDataSet(q);
                            if (dscash.Tables[0].Rows.Count > 0)
                            {
                ...
                {
                    ds = new DataSet();
                    q = "SELECT     SUM(dbo.BillType.Amount) AS cash, count(dbo.BillType.id) as count  FROM         dbo.BillType INNER JOIN dbo.Sale ON dbo.BillType.saleid = dbo.Sale.Id where (dbo.Sale.Date = '" + dateTimePicker2.Text + "') and dbo.BillType.type='Cash'  and dbo.Sale.shiftid='" + comboBox1.SelectedValue + "' and  dbo.Sale.Terminal='" + cmbterminal.Text.ToString() + "'  and dbo.Sale.BillStatus='Paid'";
                ...
                {
                    ds = new DataSet();
                    q = "SELECT     SUM(dbo.BillType.Amount) AS cash  FROM         dbo.BillType INNER JOIN dbo.Sale ON dbo.BillType.saleid = dbo.Sale.Id where (dbo.Sale.Date = '" + dateTimePicker2.Text + "') and dbo.BillType.type='Master Card'  and dbo.Sale.shiftid='" + comboBox1.SelectedValue + "' and  dbo.Sale.Terminal='" + cmbterminal.Text.ToString() + "'  and dbo.Sale.BillStatus='Paid'";
                ...
                {
                    ds = new DataSet();
                    q = "SELECT     SUM(dbo.BillType.Amount) AS cash, dbo.BillType.type  FROM         dbo.BillType INNER JOIN dbo.Sale ON dbo.BillType.saleid = dbo.Sale.Id where (dbo.Sale.Date = '" + dateTimePicker2.Text + "') and dbo.BillType.type like '%Visa%'   and dbo.Sale.shiftid='" + comboBox1.SelectedValue + "' and  dbo.Sale.Terminal='" + cmbterminal.Text.ToString() + "' and dbo.Sale.BillStatus='Paid' GROUP BY dbo.BillType.type";
                ...
                {
                    ds = new DataSet();
                    q = "SELECT     SUM(dbo.BillType.Amount) AS cash  FROM         dbo.BillType INNER JOIN dbo.Sale ON dbo.BillType.saleid = dbo.Sale.Id where (dbo.Sale.Date = '" + dateTimePicker2.Text + "') and dbo.BillType.type='Receivable'  and dbo.Sale.shiftid='" + comboBox1.SelectedValue + "' and  dbo.Sale.Terminal='" + cmbterminal.Text.ToString() + "'  and dbo.Sale.BillStatus='Paid'";
                    //q = "SELECT     SUM(dbo.Sale.TotalBill + dbo.Sale.GST - dbo.Sale.DiscountAmount) AS cash  FROM  Sale where (dbo.Sale.Date = '" + dateTimePicker2.Text + "') and dbo.Sale.BillType='Credit Card' and dbo.Sale.shiftid='" + comboBox1.SelectedValue + "' and dbo.Sale.BillStatus='Paid'";
                    ds = objCore.funGetDataSet(q);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                ...
                {
                    ds = new DataSet();
                    q = "SELECT     SUM(dbo.BillType.Amount) AS cash  FROM         dbo.BillType INNER JOIN dbo.Sale ON dbo.BillType.saleid = dbo.Sale.Id where (dbo.Sale.Date = '" + dateTimePicker2.Text + "') and dbo.BillType.type='Receivable' and dbo.Sale.shiftid='" + comboBox1.SelectedValue + "' and  dbo.Sale.Terminal='" + cmbterminal.Text.ToString() + "' and dbo.Sale.BillStatus='Paid'  and dbo.Sale.GSTtype='Cash'";
                ...
                {
                    ds = new DataSet();
                    q = "SELECT     SUM(dbo.BillType.Amount) AS cash  FROM         dbo.BillType INNER JOIN dbo.Sale ON dbo.BillType.saleid = dbo.Sale.Id where (dbo.Sale.Date = '" + dateTimePicker2.Text + "') and dbo.BillType.type='Receivable' and dbo.Sale.shiftid='" + comboBox1.SelectedValue + "' and  dbo.Sale.Terminal='" + cmbterminal.Text.ToString() + "' and dbo.Sale.BillStatus='Paid'  and dbo.Sale.GSTtype='Card'";
                    //q = "SELECT     SUM(dbo.Sale.TotalBill + dbo.Sale.GST - dbo.Sale.DiscountAmount) AS cash  FROM  Sale where (dbo.Sale.Date between '" + dateTimePicker1.Text + "' and '" + dateTimePicker2.Text + "') and dbo.Sale.BillType='Credit Card' and dbo.Sale.BillStatus='Paid'";
                ...
                {
                    ds = new DataSet();
                    q = "SELECT   * FROM  shiftcash where  (Date = '" + dateTimePicker2.Text + "') and shiftid='" + comboBox1.SelectedValue + "' and Terminal='" + cmbterminal.Text + "'";
                    ds = objCore.funGetDataSet(q);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                ...
                {
                    ds = new DataSet();
                    q = "SELECT     SUM(NetBill) AS cash , count(id) as count FROM         Sale where  (Date = '" + dateTimePicker2.Text + "') and shiftid='" + comboBox1.SelectedValue + "'and Terminal='" + cmbterminal.Text.ToString() + "' and billstatus='Paid' and OrderType='Delivery'";
                    ds = objCore.funGetDataSet(q);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        temp = ds.Tables[0].Rows[0]["cash"].ToString();
                ...
                {
                    ds = new DataSet();
                    q = "SELECT     SUM(NetBill) AS cash, count(id) as count  FROM         Sale where  (Date = '" + dateTimePicker2.Text + "') and shiftid='" + comboBox1.SelectedValue + "'and Terminal='" + cmbterminal.Text.ToString() + "' and OrderType='Take Away' and BillStatus='Paid'";
                    ds = objCore.funGetDataSet(q);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                ...
                {
                    ds = new DataSet();
                    q = "SELECT     SUM(NetBill) AS cash, count(id) as count  FROM         Sale where  (Date = '" + dateTimePicker2.Text + "') and shiftid='" + comboBox1.SelectedValue + "' and OrderType='Dine In' and Terminal='" + cmbterminal.Text.ToString() + "' and BillStatus='Paid'";
                    ds = objCore.funGetDataSet(q);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        try
                        {
                            ...
                            q = "SELECT        SUM(CAST(COALESCE(guests, 0) AS int)) AS count FROM            dbo.Sale INNER JOIN                         dbo.DinInTables ON dbo.Sale.Id = dbo.DinInTables.Saleid  where (dbo.Sale.Date =  '" + dateTimePicker2.Text + "') and shiftid='" + comboBox1.SelectedValue + "'  and Terminal='" + cmbterminal.Text.ToString() + "'  and dbo.Sale.OrderType='Dine In' and dbo.Sale.BillStatus='Paid'";
                ...
                {
                    ds = new DataSet();
                    q = "SELECT     SUM(TotalBill + GST - DiscountAmount) AS cash, count(id) as count  FROM         Sale where  (Date = '" + dateTimePicker2.Text + "') and Terminal='" + cmbterminal.Text.ToString() + "' and BillStatus='Refund' and shiftid='" + comboBox1.SelectedValue + "'";
                    ds = objCore.funGetDataSet(q);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                ...
                {
                    ds = new DataSet();
                    q = "SELECT SUM(dbo.Saledetailsrefund.Price + dbo.Saledetailsrefund.ItemGst - dbo.Saledetailsrefund.ItemdiscountPerc) AS Expr1 FROM  dbo.Saledetailsrefund INNER JOIN               dbo.Sale ON dbo.Saledetailsrefund.saleid = dbo.Sale.Id where dbo.Saledetailsrefund.type='Void' and  (dbo.Sale.Date = '" + dateTimePicker2.Text + "' )  and sale.Terminal='" + cmbterminal.Text.ToString() + "'  and shiftid='" + comboBox1.SelectedValue + "'";
                    ds = objCore.funGetDataSet(q);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
            ...
            {
                ds = new DataSet();
                string q = "SELECT     count(id) AS cash  FROM         Sale where  (Date = '" + dateTimePicker2.Text + "') and BillStatus='Paid' and shiftid='" + comboBox1.SelectedValue + "' and terminal='" + cmbterminal.Text + "'";
                ds = objCore.funGetDataSet(q);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    //refund = Convert.ToDouble(ds.Tables[0].Rows[0]["cash"].ToString());
        ...
        private void RptUserSale_Load(object sender, EventArgs e)
        {
            ...
            {
                DataSet ds = new DataSet();
                string q = "select distinct terminal from sale order by terminal ";
                ds = objCore.funGetDataSet(q);
                //DataRow dr = ds.Tables[0].NewRow();
                //dr["name"] = "All Shifts";
                //ds.Tables[0].Rows.Add(dr);
                cmbterminal.DataSource = ds.Tables[0];
                cmbterminal.ValueMember = "terminal";
                cmbterminal.DisplayMember = "terminal";
        ...
        public string printtype()
        {
            ...
            try
            {
                string q = "select * from printtype where terminal='" + cmbterminal.Text + "' and printer is null or terminal='" + cmbterminal.Text + "' and printer='receipt'";
        ...
        private void PrintReceipt(string cashier, string date, string shift, string type)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    q = "SELECT     SUM(dbo.Saledetails.Price  - dbo.Saledetails.Itemdiscount) AS sum, COUNT(dbo.Saledetails.Price) AS count, dbo.MenuGroup.Name FROM         dbo.Saledetails INNER JOIN                      dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id INNER JOIN                      dbo.MenuGroup ON dbo.MenuItem.MenuGroupId = dbo.MenuGroup.Id INNER JOIN                      dbo.Sale ON dbo.Saledetails.saleid = dbo.Sale.Id WHERE     (Sale.Date = '" + date + "') and  dbo.MenuGroup.Id='" + ds.Tables[0].Rows[i]["Id"].ToString() + "'  and Sale.shiftid='" + shift + "' and sale.Terminal='" + cmbterminal.Text.ToString() + "'  and dbo.sale.billtype like 'Cash%' and (Sale.BillStatus = 'Paid') GROUP BY dbo.MenuGroup.Name ";
                    DataSet dscash = new DataSet();
                    dscash = objCore.funGetDataSet(q);
                    if (dscash.Tables[0].Rows.Count > 0)
                    {
                    ...
                    q = "SELECT     SUM(dbo.Saledetails.Price  - dbo.Saledetails.Itemdiscount) AS sum, COUNT(dbo.Saledetails.Price) AS count, dbo.MenuGroup.Name FROM         dbo.Saledetails INNER JOIN                      dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id INNER JOIN                      dbo.MenuGroup ON dbo.MenuItem.MenuGroupId = dbo.MenuGroup.Id INNER JOIN                      dbo.Sale ON dbo.Saledetails.saleid = dbo.Sale.Id WHERE     (Sale.Date = '" + date + "') and  dbo.MenuGroup.Id='" + ds.Tables[0].Rows[i]["Id"].ToString() + "'  and Sale.shiftid='" + shift + "' and sale.Terminal='" + cmbterminal.Text.ToString() + "'  and dbo.sale.billtype like 'Visa%'  and (Sale.BillStatus = 'Paid') GROUP BY dbo.MenuGroup.Name ";
                    dscash = new DataSet();
                    dscash = objCore.funGetDataSet(q);
                    if (dscash.Tables[0].Rows.Count > 0)
                    {
                ...
                double net = 0, gst = 0, cashdis = 0, carddis = 0, dis = 0, refund = 0, gross = 0, service = 0, receivable = 0, cashgst = 0, cardgst = 0;
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        qq1 = "SELECT  dbo.BillType.Amount AS cash, dbo.BillType.type, dbo.BillType.saleid FROM  dbo.BillType INNER JOIN               dbo.Sale ON dbo.BillType.saleid = dbo.Sale.Id WHERE (dbo.BillType.type LIKE '%Visa%') AND (dbo.sale.BillStatus = 'Paid') and dbo.sale.date='" + date + "' and dbo.sale.Terminal='" + cmbterminal.Text.ToString() + "'  ORDER BY dbo.BillType.type";
                ...
                try
                {
                    ...
                    //if (type == "shift")
                    {
                        qq = "SELECT ROUND(SUM(Discountamount), 2) AS Expr1, Discount FROM  dbo.Sale WHERE (Discount > 0) AND (BillStatus = 'Paid') and date='" + date + "' and shiftid='" + shift + "' and Terminal='" + cmbterminal.Text.ToString() + "' GROUP BY Discount";
                    }
                    //else
                    //{
                    ...
                    //if (type == "shift")
                    {
                        qq = "SELECT ROUND(SUM(Discountamount), 2) AS Expr1, Discount FROM  dbo.Sale1 WHERE  date='" + date + "' AND (BillStatus = 'Paid') and Terminal='" + cmbterminal.Text.ToString() + "' and shiftid='" + shift + "' GROUP BY Discount";
                    }
                    //else
                    //{
                    //    qq = "SELECT ROUND(SUM(Discountamount), 2) AS Expr1, Discount FROM  dbo.Sale1 WHERE  date='" + date + "' AND (BillStatus = 'Paid') GROUP BY Discount";
                ...
                try
                {
                    try
                    {
                        ...
                        q = "SELECT       ((CONVERT(float, dbo.RuntimeModifier.Price))  * (dbo.Saledetails.Quantity)) AS amount, SUM(dbo.Saledetails.Quantity) AS Quantity, dbo.Sale.Id,dbo.Sale.customer, dbo.Sale.Date, dbo.RuntimeModifier.name FROM            dbo.Sale INNER JOIN                         dbo.Saledetails ON dbo.Sale.Id = dbo.Saledetails.saleid INNER JOIN                         dbo.RuntimeModifier ON dbo.Saledetails.RunTimeModifierId = dbo.RuntimeModifier.id WHERE        (dbo.Saledetails.RunTimeModifierId > 0) AND (dbo.Saledetails.ModifierId = 0) AND (dbo.Saledetails.Price = 0) AND (dbo.RuntimeModifier.price > 0) and (sale.date = '" + dateTimePicker2 + "') and dbo.sale.shiftid='" + comboBox1.SelectedValue + "' and dbo.sale.Terminal='" + cmbterminal.Text + "'  GROUP BY dbo.RuntimeModifier.Name, dbo.Sale.Id, dbo.Sale.Date,dbo.Sale.customer,dbo.RuntimeModifier.Price";
                ...
                try
                {
                    //if (type == "shift")
                    {
                        q = "SELECT     SUM(dbo.BillType.Amount) AS cash, dbo.BillType.type  FROM         dbo.BillType INNER JOIN dbo.Sale ON dbo.BillType.saleid = dbo.Sale.Id where (dbo.Sale.Date = '" + date + "') and dbo.BillType.type like '%Visa%'  and dbo.sale.Terminal='" + cmbterminal.Text.ToString() + "'  and dbo.Sale.shiftid='" + shift + "' GROUP BY dbo.BillType.type";
                    }
                    //else
                    //{
                    //    q = "SELECT     SUM(dbo.BillType.Amount) AS cash , dbo.BillType.type FROM         dbo.BillType INNER JOIN dbo.Sale ON dbo.BillType.saleid = dbo.Sale.Id where (dbo.Sale.Date = '" + date + "') and dbo.BillType.type like '%Visa%'  and dbo.sale.Terminal='" + cmbterminal.Text.ToString() + "' GROUP BY dbo.BillType.type";
                    //}
        ...
        private void PrintLineItem(OPOSPOSPrinter printer, string itemCode, double pricecash, double pricevisa, double Price)
        {

            {
                ...
                //string text = itemCode + Price.ToString().PadLeft(40 - itemCode.Length);
                //PrintTextLine(printer, text);
        ...
        private void vButton2_Click(object sender, EventArgs e)
        {
            ...
            if (type == "opos")
            {
                ...
                vButton1.Enabled = false;
                vButton2.Enabled = false;
                PrintReceipt("", dateTimePicker2.Text, comboBox1.SelectedValue.ToString(), "shift");
                vButton2.Text = "Print";
                vButton1.Enabled = true;
                vButton2.Enabled = true;
            }
            ...
            {
                Printshiftend(printername("generic"), "", dateTimePicker2.Text, comboBox1.SelectedValue.ToString(), "shift");
            }
            else
            {
                Printshiftend(printername("generic"), "", dateTimePicker2.Text, comboBox1.SelectedValue.ToString(), "shift");
        ...
        private void PrintReceiptshiftend(BinaryWriter bw, string cashier, string date, string shift, string type)
        {
           ...
           // bw.NormalFont(comp);
            ...
            if (type == "shift")
            {
                ...
                bw.NormalFont("Shift : " + shiftname + "  Terminal : " + cmbterminal.Text); 
            ...
            else
            {
                ...
                bw.NormalFont(print);
                bw.NormalFont("DATE :  " + Convert.ToDateTime(date).ToShortDateString() + " " + DateTime.Now.ToShortTimeString());
                //bw.NormalFont("Shift : " + shift + "  Terminal : " + cmbterminal.Text);
                bw.NormalFont("Name :  " + cashier);
            }
            bw.NormalFont(print);
            bw.LargeText("          Sales");
            ...
            if (printsubgroup() == "Enabled")
            {
                if (type == "shift")
                {
                    q = "SELECT     SUM(dbo.Saledetails.Price  - dbo.Saledetails.Itemdiscount) AS sum, COUNT(dbo.Saledetails.Price) AS count, dbo.MenuGroup.SubGroup as Name FROM         dbo.Saledetails INNER JOIN                      dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id INNER JOIN                      dbo.MenuGroup ON dbo.MenuItem.MenuGroupId = dbo.MenuGroup.Id INNER JOIN                      dbo.Sale ON dbo.Saledetails.saleid = dbo.Sale.Id WHERE     (Sale.Date = '" + date + "')  and  dbo.Sale.Terminal='" + cmbterminal.Text.ToString() + "' and Sale.shiftid='" + shift + "' and (Sale.BillStatus = 'Paid') and Saledetails.Price > 0 GROUP BY dbo.MenuGroup.SubGroup ";
                }
                else
                {
                    q = "SELECT     SUM(dbo.Saledetails.Price  - dbo.Saledetails.Itemdiscount) AS sum, COUNT(dbo.Saledetails.Price) AS count, dbo.MenuGroup.SubGroup as Name FROM         dbo.Saledetails INNER JOIN                      dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id INNER JOIN                      dbo.MenuGroup ON dbo.MenuItem.MenuGroupId = dbo.MenuGroup.Id INNER JOIN                      dbo.Sale ON dbo.Saledetails.saleid = dbo.Sale.Id WHERE     (Sale.Date = '" + date + "')  and  dbo.Sale.Terminal='" + cmbterminal.Text.ToString() + "'  and (Sale.BillStatus = 'Paid') and Saledetails.Price > 0 GROUP BY dbo.MenuGroup.SubGroup ";
            ...
            try
            {
                ...
                if (dsitem.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int ii = 0; ii < dsitem.Tables[0].Rows.Count; ii++)
                    {
                        ...
                        q = "select sum(QuantityConsumed) as QuantityConsumed from InventoryConsumed where date between '" + dateTimePicker2.Text + "' and '" + dateTimePicker2.Text + "' and RawItemId='" + dsitem.Tables[0].Rows[ii]["id"].ToString() + "'";
                        ds = new DataSet();
                        ds = objCore.funGetDataSet(q);
                        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                        {
            ...
            double net = 0, gst = 0, cashdis = 0,posfee=0, carddis = 0, dis = 0, refund = 0, voidsale = 0, gross = 0,expense=0, cashgst = 0, cardgst = 0, avgdinein = 0, avgdineintable = 0;
            double cash = 0, credit = 0, master = 0, checks = 0, totalchks = 0, delivery = 0, membership = 0, visa = 0, carhope = 0, calculatedcash = 0, drawerfloat = 0, bankingtotal = 0, declared = 0, over = 0, totalcash = 0;
            if (dt.Rows.Count > 0)
            {
                try
                {
                    if (type == "shift")
                    {
                        q = "select sum(amount) as amount from CashierExpenses where date='" + date + "' and terminal='" + cmbterminal.Text + "' and shiftid='" + shift + "'";
                    }
                    else
                    {
            ...
            try
            {
                ...
                //if (type == "shift")
                //{
                qq = "SELECT ROUND(SUM(Discountamount), 2) AS Expr1, Discount FROM  dbo.Sale WHERE (Discount > 0) AND (BillStatus = 'Paid') and date='" + dateTimePicker2.Text + "' and shiftid='" + comboBox1.SelectedValue + "' GROUP BY Discount";
                //}
                ////else
                //{
            ...
            try
            {
                try
                {
                    q = "SELECT        ((CONVERT(float, dbo.MenuItem.Price)) + (CONVERT(float, ISNULL(dbo.ModifierFlavour.price, '0')))) * SUM(dbo.Saledetails.Quantity) AS amount, SUM(dbo.Saledetails.Quantity) AS Quantity, dbo.MenuItem.Name, dbo.Sale.Id,dbo.Sale.customer, dbo.Sale.Date, dbo.ModifierFlavour.name AS Expr1 FROM            dbo.Sale INNER JOIN                         dbo.Saledetails ON dbo.Sale.Id = dbo.Saledetails.saleid INNER JOIN                         dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id LEFT OUTER JOIN                         dbo.ModifierFlavour ON dbo.Saledetails.Flavourid = dbo.ModifierFlavour.Id WHERE        (dbo.Saledetails.RunTimeModifierId = 0) AND (dbo.Saledetails.ModifierId = 0) AND (dbo.Saledetails.Price = 0)  and (sale.date ='" + dateTimePicker2.Text + "')  and dbo.Sale.shiftid='" + shift + "' GROUP BY dbo.MenuItem.Name, dbo.Sale.Id, dbo.Sale.Date, dbo.ModifierFlavour.name,dbo.Sale.customer, dbo.MenuItem.Price, dbo.ModifierFlavour.price";
                    q = "SELECT        ((CONVERT(float, dbo.MenuItem.Price)) + (CONVERT(float, ISNULL(dbo.ModifierFlavour.price, '0')))) * SUM(dbo.Saledetails.Quantity) AS amount, SUM(dbo.Saledetails.Quantity) AS Quantity, dbo.MenuItem.Name, dbo.Sale.Id,dbo.Sale.customer, dbo.Sale.Date, dbo.ModifierFlavour.name AS Expr1 FROM            dbo.Sale INNER JOIN                         dbo.Saledetails ON dbo.Sale.Id = dbo.Saledetails.saleid INNER JOIN                         dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id LEFT OUTER JOIN                         dbo.ModifierFlavour ON dbo.Saledetails.Flavourid = dbo.ModifierFlavour.Id WHERE        (dbo.Saledetails.RunTimeModifierId = 0) AND (dbo.Saledetails.ModifierId = 0) AND (dbo.Saledetails.Price = 0) AND ((CONVERT(float, dbo.MenuItem.Price)) + (CONVERT(float, ISNULL(dbo.ModifierFlavour.price, '0')))) > 0 and (sale.date =  '" + dateTimePicker2.Text + "')  and dbo.Sale.shiftid='" + shift + "'  GROUP BY dbo.MenuItem.Name, dbo.Sale.Id, dbo.Sale.Date, dbo.ModifierFlavour.name,dbo.Sale.customer, dbo.MenuItem.Price, dbo.ModifierFlavour.price order by dbo.Sale.Date";
                    ...
                    q = "SELECT       ((CONVERT(float, dbo.RuntimeModifier.Price))  * SUM(dbo.Saledetails.Quantity)) AS amount, SUM(dbo.Saledetails.Quantity) AS Quantity, dbo.Sale.Id,dbo.Sale.customer, dbo.Sale.Date, dbo.RuntimeModifier.name FROM            dbo.Sale INNER JOIN                         dbo.Saledetails ON dbo.Sale.Id = dbo.Saledetails.saleid INNER JOIN                         dbo.RuntimeModifier ON dbo.Saledetails.RunTimeModifierId = dbo.RuntimeModifier.id WHERE        (dbo.Saledetails.RunTimeModifierId > 0) AND (dbo.Saledetails.ModifierId = 0) AND (dbo.Saledetails.Price = 0) AND (dbo.RuntimeModifier.price > 0) and (sale.date ='" + dateTimePicker2.Text + "')  and dbo.Sale.shiftid='" + shift + "' GROUP BY dbo.RuntimeModifier.Name, dbo.Sale.Id, dbo.Sale.Date,dbo.Sale.customer,dbo.RuntimeModifier.Price";
                    DataSet dsdis1 = new DataSet();
                    dsdis1 = new DataSet();
                    dsdis1 = objCore.funGetDataSet(q);
                    IList<complimentoryClass> data1 = dsdis1.Tables[0].AsEnumerable().Select(row =>
                ...
                //    q = "SELECT        (SUM(CONVERT(float, dbo.MenuItem.Price)) + SUM(CONVERT(float, ISNULL(dbo.ModifierFlavour.price, '0')))) * SUM(dbo.Saledetails.Quantity) AS Expr1, SUM(dbo.Saledetails.Quantity) AS Expr2 FROM  dbo.Sale INNER JOIN               dbo.Saledetails ON dbo.Sale.Id = dbo.Saledetails.saleid INNER JOIN               dbo.MenuItem ON dbo.Saledetails.MenuItemId = dbo.MenuItem.Id LEFT OUTER JOIN               dbo.ModifierFlavour ON dbo.Saledetails.Flavourid = dbo.ModifierFlavour.Id  WHERE dbo.Saledetails.price='0' AND (dbo.Sale.Discount < 100)  and dbo.Sale.date='" + dateTimePicker2.Text + "' and shiftid='" + comboBox1.SelectedValue + "'  AND (dbo.Sale.BillStatus = 'Paid')";
           ...
           double service = Convert.ToDouble(temp);
            ...
            for (int i = 0; i < dsBanks.Tables[0].Rows.Count; i++)
            {
                ...
                if (type == "shift")
                {
                    q = "SELECT     SUM(dbo.Sale.NetBill) AS cash ,count(*) as p  FROM         Sale where (Date = '" + date + "') and BillType='Visa " + dsBanks.Tables[0].Rows[i]["Name"] + "' and shiftid='" + shift + "' and BillStatus='Paid' and Terminal='" + cmbterminal.Text + "'";
                }
                else
                {
                    q = "SELECT     SUM(dbo.Sale.NetBill) AS cash ,count(*) as p  FROM         Sale where (Date = '" + date + "') and BillType='Visa " + dsBanks.Tables[0].Rows[i]["Name"] + "'  and BillStatus='Paid'";
            ...
            //if (type == "shift")
            //{
            //    q = "SELECT     SUM(dbo.Sale.TotalBill + dbo.Sale.GST - dbo.Sale.DiscountAmount) AS cash ,count(*) as p  FROM         Sale where (Date = '" + date + "') and BillType='Receivable' and shiftid='" + shift + "' and BillStatus='Paid' and Terminal='" + cmbterminal.Text + "'";
            //}
            //else
            //{
            //    q = "SELECT     SUM(dbo.Sale.TotalBill + dbo.Sale.GST - dbo.Sale.DiscountAmount) AS cash ,count(*) as p  FROM         Sale where (Date = '" + date + "') and BillType='Receivable'  and BillStatus='Paid' and Terminal='" + cmbterminal.Text + "'";

            //}
            if (type == "shift")
            {
               
                q = "SELECT        SUM(dbo.Sale.TotalBill + dbo.Sale.GST - dbo.Sale.DiscountAmount) AS cash, COUNT(*) AS p, dbo.Customers.Name FROM            dbo.Sale INNER JOIN                         dbo.BillType ON dbo.Sale.Id = dbo.BillType.saleid INNER JOIN                         dbo.Customers ON dbo.BillType.recvid = dbo.Customers.Id where (dbo.Sale.Date  = '" + date + "' ) and dbo.Sale.shiftid='" + shift + "'  and dbo.Sale.BillType='Receivable'  and dbo.Sale.BillStatus='Paid' and dbo.Sale.Terminal='" + cmbterminal.Text + "' GROUP BY dbo.Customers.Name";
            ...
            //nm = "Master Sale : (" + masterp + ")";
            //for (int j = nm.Length; j < space.Length; j++)
            //{
            //    nm += " ";
            //}
            //temp = Math.Round(master, 3).ToString();
            //if (!temp.Contains("."))
            //{
            //    temp = temp + ".00";
            //}

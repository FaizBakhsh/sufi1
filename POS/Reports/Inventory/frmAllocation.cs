    ...
    public partial class frmAllocation : Form
    {
        ...
        private void frmClosingInventory_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                string q = "select id,branchname from branch where BranchName like '%ware%' ";
        ...
        public void bindreport()
        {
            ...
            try
            {
                ...
                POSRestaurant.Reports.Inventory.rptAllocation rptDoc = new rptAllocation();
                POSRestaurant.Reports.Inventory.dsAllocation dsrpt = new  dsAllocation();
                //feereport ds = new feereport(); // .xsd file name
                getcompany();
                dt = getAllOrders(rptDoc);
                ...
                string q = "SELECT        TOP (100) PERCENT dbo.Branch.Id, dbo.Branch.BranchName, dbo.Branch.BranchCode, dbo.InventoryAllocation.Percentage FROM            dbo.Branch INNER JOIN                         dbo.InventoryAllocation ON dbo.Branch.Id = dbo.InventoryAllocation.branchid WHERE        (dbo.Branch.type = 'Take Away') OR                         (dbo.Branch.type = 'Dine In') AND (dbo.Branch.Status = 'Active') ORDER BY dbo.Branch.BranchName";
                DataSet dsbranches = new DataSet();
                dsbranches = objcore.funGetDataSet(q);
                List<TextObject> textObjects = rptDoc.ReportDefinition.Sections["Section2"].ReportObjects.OfType<TextObject>().ToList();
                for (int ii = 3; ii < textObjects.Count; ii++)
                {
                    //Set the name of Column in TextObject.
                    textObjects[ii].Text = string.Empty;
                    if (dsbranches.Tables[0].Rows.Count > ii-3)
                    {
                        textObjects[ii].Text = dsbranches.Tables[0].Rows[ii-3][2].ToString();
                    }
                }
                rptDoc.SetParameterValue("Comp", cmbbranch.Text);
        ...
        public double opening(string itemid)
        {
            ...
            {
                q = "SELECT     SUM(dbo.PurchaseDetails.TotalItems) AS Expr1 FROM         dbo.Purchase INNER JOIN                      dbo.PurchaseDetails ON dbo.Purchase.Id = dbo.PurchaseDetails.PurchaseId where  BranchCode='" + cmbbranch.SelectedValue + "' and  dbo.Purchase.date between '" + start + "' and  '" + end + "' and dbo.PurchaseDetails.RawItemId='" + itemid + "'";
        ...
        public DataTable getAllOrders(ReportDocument crystalReport)
        {
            ...
            try
            {
                ...
                dtrpt.Columns.Add("DataColumn4", typeof(string));
                dtrpt.Columns.Add("DataColumn5", typeof(string));
                dtrpt.Columns.Add("DataColumn6", typeof(string));
                dtrpt.Columns.Add("DataColumn7", typeof(string));
                dtrpt.Columns.Add("DataColumn8", typeof(string));
                dtrpt.Columns.Add("DataColumn9", typeof(string));
                dtrpt.Columns.Add("DataColumn10", typeof(string));
                dtrpt.Columns.Add("DataColumn11", typeof(string));
                dtrpt.Columns.Add("DataColumn12", typeof(string));
                dtrpt.Columns.Add("DataColumn13", typeof(string));
                dtrpt.Columns.Add("DataColumn14", typeof(string));
                dtrpt.Columns.Add("DataColumn15", typeof(string));
                dtrpt.Columns.Add("DataColumn16", typeof(string));
                dtrpt.Columns.Add("DataColumn17", typeof(string));
                ...
                double purchased = 0, consumed = 0, variance = 0, price = 0, discard = 0, tint = 0, tout = 0, minorder = 0,max=0,balance=0;
                DataSet ds = new DataSet();
                string q = "", date = "" ;
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    {
                        q = "SELECT     SUM(dbo.PurchaseDetails.TotalItems) AS Expr1 FROM         dbo.Purchase INNER JOIN                      dbo.PurchaseDetails ON dbo.Purchase.Id = dbo.PurchaseDetails.PurchaseId where dbo.Purchase.BranchCode ='" + cmbbranch.SelectedValue + "'  and dbo.Purchase.date ='" + date + "'  and dbo.PurchaseDetails.RawItemId='" + ds.Tables[0].Rows[i]["id"].ToString() + "'";
                    }
                        dspurchase = objCore.funGetDataSet(q);
                        if (dspurchase.Tables[0].Rows.Count > 0)
                        {
                    ...
                    try
                    {
                        ...
                        {
                            q = "SELECT     SUM(dbo.Production.Quantity) AS Expr1 FROM         Production  where branchid='"+cmbbranch.SelectedValue+"' and (dbo.Production.date = '" + dateTimePicker1.Text + "' ) and dbo.Production.ItemId='" + ds.Tables[0].Rows[i]["rid"].ToString() + "' and dbo.Production.status='Posted'";
                   ...
                   {
                       q = "SELECT     SUM(TransferIn) AS Expr1 FROM     InventoryTransfer where Date ='" + date + "' and branchid='"+cmbbranch.SelectedValue+"' and itemid='" + ds.Tables[0].Rows[i]["id"].ToString() + "'";
                   }
                    ...
                    if (checkBox1.Checked == false)
                    {
                        ...
                        q = "SELECT        dbo.Branch.Id, dbo.Branch.BranchName, dbo.Branch.BranchCode, dbo.InventoryAllocation.Percentage FROM            dbo.Branch INNER JOIN                         dbo.InventoryAllocation ON dbo.Branch.Id = dbo.InventoryAllocation.branchid WHERE        (dbo.Branch.type = 'Take Away') OR                         (dbo.Branch.type = 'Dine In') AND (dbo.Branch.Status = 'Active') ORDER BY dbo.Branch.BranchName";
                        DataSet dsbranches = new DataSet();
                        dsbranches = objcore.funGetDataSet(q);


                        DataRow dr = dtrpt.NewRow();
                        dr[0] = ds.Tables[0].Rows[i]["ItemName"].ToString();
                        dr[1] = closing;
                        dr[2] = ds.Tables[0].Rows[i]["UOM"].ToString();

                        try
                        {
                            for (int k = 0; k < dsbranches.Tables[0].Rows.Count; k++)
                            {
                                string tempperc = dsbranches.Tables[0].Rows[k][3].ToString();
                                double perc = double.Parse(tempperc);
                                perc = Math.Round(closing * perc / 100, 2);
                                dr[k + 3] = perc;
                            }
                        }
                        catch (Exception ex)
        ...
        private void InitializeComponent()
        {
            ...
            this.vButton1.Location = new System.Drawing.Point(218, 94);
            ...
            this.Text = "Inventory Allocation Report";

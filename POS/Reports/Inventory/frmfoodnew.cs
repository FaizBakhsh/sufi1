    ...
    public partial class frmfoodnew : Form
    {
        ...
        public void bindreport()
        {
            ...
            try
            {
                ...
                POSRestaurant.Reports.Inventory.rptSKUCritical rptDocSku= new rptSKUCritical();;
                POSRestaurant.Reports.Inventory.rptfood rptDoc = new rptfood(); 
                if (cbSku.Checked)
                {

                  rptDocSku = new rptSKUCritical();
                }
                else
                {
                   rptDoc = new rptfood();
                ...
                if (cbSku.Checked)
                {
                    rptDocSku.SetDataSource(dsrpt);
                    rptDocSku.SetParameterValue("Comp", company);
                    rptDocSku.SetParameterValue("address", phone);
                    rptDocSku.SetParameterValue("phone", address);
                    rptDocSku.SetParameterValue("date", " For the period of " + dateTimePicker1.Text + " to " + dateTimePicker2.Text);
                    rptDocSku.SetParameterValue("closing", closingamount);
                    rptDocSku.SetParameterValue("waste", wastage);
                    if (type == "value")
                    {
                        rptDocSku.SetParameterValue("title", "Critical Inventory Report(Value)");
                    }
                    else
                    {
                        rptDocSku.SetParameterValue("title", "Critical Inventory Report(Quantitative)");
                    }
                    crystalReportViewer1.ReportSource = rptDocSku;
                }
                else
                {
        ...
        public double openingclosing(string itemid,string date,double closing)
        {
            ...
            if (dspurchase.Tables[0].Rows.Count > 0)
            {
                ...
                ClosingDate = Convert.ToDateTime(date2);
                if (val == "")
                {
                    val = "0";
                }
        ...
        public void fillopening()
        {
            ...
            if (cmbkitchen.Text == "All" && checkBox1.Checked == false)
            {
                if (cmbbranch1.Text == "All")
                {
                    ...
                    q = "SELECT top 1 date, (remaining) as rem FROM     closing where  Date <'" + date + "' and itemid='" + cmbitem.SelectedValue + "'  and kdsid is null  order by Date desc";

                }
                else
                {

                    q = "SELECT top 1 date, (remaining) as rem FROM     closing where branchid='" + cmbbranch1.SelectedValue + "' and Date <'" + date + "' and itemid='" + cmbitem.SelectedValue + "'  and kdsid is null  order by Date desc";
            ...
            else
            {
                if (checkBox1.Checked == true)
                {
                    q = "SELECT top 1 date, (remaining) as rem FROM     closing where branchid='" + cmbbranch1.SelectedValue + "' and Date <'" + date + "' and itemid='" + cmbitem.SelectedValue + "' and   isnull(kdsid,'') != '' order by Date desc";
                }
                else
                {
                    q = "SELECT top 1 date, (remaining) as rem FROM     closing where branchid='" + cmbbranch1.SelectedValue + "' and Date <'" + date + "' and itemid='" + cmbitem.SelectedValue + "' and kdsid='" + cmbkitchen.SelectedValue + "' order by Date desc";
                }
            }
            dspurchase = objcore.funGetDataSet(q);
            if (dspurchase.Tables[0].Rows.Count > 0)
           ...
           // DateTime end1 = Convert.ToDateTime(date2);
            ...
            {
                start = ClosingDate.Value.ToString("yyyy-MM-dd");
            }
            else
            {
                start = Convert.ToDateTime(dateTimePicker1.Text).AddMonths(-2).ToString("yyyy-MM-dd");
            }
            //string start = Convert.ToDateTime(dateTimePicker1.Text).ToString("yyyy-MM-dd");
        ...
        //        string qs = "select Code from RawItem where Id='" + itemid + "'";
        //        string Code = "";
        //        dschk = objCore.funGetDataSet(qs);
        //    if (dschk.Tables[0].Rows.Count > 0)
        //    {

        //        Code = Convert.ToString(dschk.Tables[0].Rows[0]["Code"]);
        ...
        //        if (logo == "")
        //        {
        //            dtrpt.Rows.Add(name, openin.ToString(), purchased.ToString(), consumed.ToString(), discard.ToString(), staff, cmplt, tint, tout, closing, minorder, balance, null, price, openingval, closingval, purchaseval, saleval, discardval, staffval, comptval, ideal, category,Code);
        //        }

        //        else
        //        {
        //            dtrpt.Rows.Add(name, openin.ToString(), purchased.ToString(), consumed.ToString(), discard.ToString(), staff, cmplt, tint, tout, closing, minorder, balance, dscompany.Tables[0].Rows[0]["logo"], price, openingval, closingval, purchaseval, saleval, discardval, staffval, comptval, ideal, category,Code);
                    ...
                    if (cmbkitchen.Text == "All")
                    {
                        if (cmbitem.Text == "All")
                        {
                            if (cmbgroup.Text == "All")
                            {
                                ...
                                q = "SELECT  dbo.RawItem.Id, dbo.RawItem.ItemName, cast( dbo.RawItem.Price as varchar(200)),case when t.Transferprice is null then Price else t.Transferprice end as price,dbo.UOM.UOM, cast( dbo.RawItem.MinOrder as varchar(200)) as MinOrder, dbo.Category.CategoryName FROM  dbo.RawItem INNER JOIN    dbo.UOM ON dbo.RawItem.UOMId = dbo.UOM.Id  INNER JOIN   dbo.Category ON dbo.RawItem.CategoryId = dbo.Category.Id left join (select Itemid, MAX(isnull(price,0)) Transferprice from InventoryTransfer where TransferIn>0 and Date between '" + start1 + "' and '" + end1 + "' group by Itemid ) t on t.Itemid=dbo.RawItem.Id where dbo.RawItem.Status='Active' or dbo.RawItem.Status is NULL order by dbo.RawItem.ItemName ";
                        ...
                        try
                        {
                            ...
                            IList<RawItemClass> data = ds1.Tables[0].AsEnumerable().Select(row =>
                                new RawItemClass
                                {
                                    ...
                                    //Price = Convert.ToString(row.Field<double>("Price")),
        ...
        //                    {
        //                        //Query for All options
        //                        q = "SELECT  dbo.RawItem.Id, dbo.RawItem.ItemName, cast( dbo.RawItem.Price as varchar(200)),case when t.Transferprice is null then 0 else t.Transferprice end as price,dbo.UOM.UOM, cast( dbo.RawItem.MinOrder as varchar(200)) as MinOrder, dbo.Category.CategoryName, dbo.RawItem.Code FROM  dbo.RawItem INNER JOIN    dbo.UOM ON dbo.RawItem.UOMId = dbo.UOM.Id  INNER JOIN   dbo.Category ON dbo.RawItem.CategoryId = dbo.Category.Id left join (select Itemid, MAX(isnull(price,0)) Transferprice from InventoryTransfer where TransferIn>0 and Date between '" + start1 + "' and '" + end1 + "' group by Itemid ) t on t.Itemid=dbo.RawItem.Id where dbo.RawItem.Status='Active' or dbo.RawItem.Status is NULL order by dbo.RawItem.ItemName ";
        //                            //q = "SELECT        dbo.RawItem.Id, dbo.RawItem.ItemName, cast( dbo.RawItem.Price as varchar(200)) as Price, dbo.UOM.UOM, cast( dbo.RawItem.MinOrder as varchar(200)) as MinOrder, dbo.Category.CategoryName FROM            dbo.RawItem INNER JOIN                         dbo.UOM ON dbo.RawItem.UOMId = dbo.UOM.Id  INNER JOIN                         dbo.Category ON dbo.RawItem.CategoryId = dbo.Category.Id  where dbo.RawItem.Status='Active' or dbo.RawItem.Status is NULL order by dbo.RawItem.ItemName";
        //                    }
        //                    else
        //                    {
        ...
        //                else
        //                {
        //                    q = "SELECT        dbo.RawItem.Id, dbo.RawItem.ItemName, cast( dbo.RawItem.Price as varchar(200)) as Price, dbo.UOM.UOM, cast( dbo.RawItem.MinOrder as varchar(200)) as MinOrder, dbo.Category.CategoryName,dbo.RawItem.Code FROM            dbo.RawItem INNER JOIN                         dbo.UOM ON dbo.RawItem.UOMId = dbo.UOM.Id INNER JOIN                         dbo.Category ON dbo.RawItem.CategoryId = dbo.Category.Id  where dbo.RawItem.Id='" + cmbitem.SelectedValue + "' order by dbo.RawItem.ItemName";
        //                }
        //            }
        //            else
        //            {
        ...
        //                            ItemName = row.Field<string>("ItemName"),
        //                           Price= "0",
        //                           // Price = Convert.ToString(row.Field<float>("Price")),
        //                            UOM = row.Field<string>("UOM") ,
        //                           MinOrder =  "0",//Convert.ToString(row.Field<double>("MinOrder"))
        //                            //,
        //                           Cat = row.Field<string>("CategoryName")

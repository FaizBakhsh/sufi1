...
namespace POSRestaurant.Reports.Accounts
{
    public partial class frmProfitLoss : Form
    {
        ...
        public DataTable getAllOrdersfinal()
        {
           string demandno = "", kdsid = "";
            ...
            if (cbIssue.Checked)
            {
                ...
                //Stock Issuance
            //    double Cost = 0; 
            //    qr = "SELECT        dbo.RawItem.ItemName,dbo.RawItem.id, dbo.UOM.UOM, dbo.InventoryTransferStore.Quantity FROM            dbo.UOM INNER JOIN                         dbo.RawItem ON dbo.UOM.Id = dbo.RawItem.UOMId INNER JOIN                         dbo.InventoryTransferStore ON dbo.RawItem.Id = dbo.InventoryTransferStore.Itemid WHERE        (dbo.InventoryTransferStore.Date = '" + Convert.ToDateTime(dateTimePicker1.Text) + "') AND (dbo.InventoryTransferStore.Invoiceno = '" + demandno + "') AND (dbo.InventoryTransferStore.RecvStoreId = '" + kdsid + "') and dbo.InventoryTransferStore.Quantity>0";
            //    dsaccounts = objCore.funGetDataSet(qr);
            //    for (int i = 0; i < dsaccounts.Tables[0].Rows.Count; i++)
            //    {
            //        string temp = dsaccounts.Tables[0].Rows[i]["Quantity"].ToString();
            //        if (temp == "")
            //        {
            //            temp = "0";
            //        }
            //        double qty = Convert.ToDouble(temp);
            //        double price = getprice(dsaccounts.Tables[0].Rows[i]["id"].ToString());
            //        Cost += (qty * price);
            //       // dtrpt.Rows.Add("2. Cost of Sales", "", "Food Cost", foodcost, dscompany.Tables[0].Rows[0]["logo"]);
            //        //dtrpt.Rows.Add(dsaccounts.Tables[0].Rows[i]["ItemName"].ToString(), dsaccounts.Tables[0].Rows[i]["UOM"].ToString(), qty, price, price * qty, dscompany.Tables[0].Rows[0]["logo"]);
            //    }

            //    qr = "SELECT        dbo.RawItem.ItemName,dbo.RawItem.id, dbo.UOM.UOM, dbo.InventoryTransferStore.Quantity FROM            dbo.UOM INNER JOIN                         dbo.RawItem ON dbo.UOM.Id = dbo.RawItem.UOMId INNER JOIN                         dbo.InventoryTransferStore ON dbo.RawItem.Id = dbo.InventoryTransferStore.Itemid WHERE        (dbo.InventoryTransferStore.Date = '" + Convert.ToDateTime(dateTimePicker1.Text) + "') AND (dbo.InventoryTransferStore.Invoiceno = '" + demandno + "') AND (dbo.InventoryTransferStore.RecvStoreId = '" + kdsid + "') and dbo.InventoryTransferStore.Quantity>0";

                //qr = "SELECT  SUM(ITS.Quantity * RI.Price) AS TotalAmount From InventoryTransferStore ITS JOIN     RawItem RI ON ITS.ItemId = RI.Id WHERE    ITS.Date BETWEEN @StartDate AND @EndDate    AND ITS.branchid = @BranchId";
            /////////////////////Store transfer//////////////////////////
              qr = "SELECT        TOP (100) PERCENT SUM(dbo.InventoryTransferStore.Quantity) AS QuantityConsumed, dbo.RawItem.Id AS rid, dbo.RawItem.Code, dbo.RawItem.ItemName, dbo.RawItem.Description, dbo.UOM.UOM,    dbo.Type.TypeName FROM            dbo.InventoryTransferStore INNER JOIN                         dbo.RawItem ON dbo.InventoryTransferStore.ItemId = dbo.RawItem.Id INNER JOIN                         dbo.UOM ON dbo.RawItem.UOMId = dbo.UOM.Id INNER JOIN                        dbo.Type ON dbo.RawItem.TypeId = dbo.Type.Id WHERE     (dbo.InventoryTransferStore.Date between '" + dateTimePicker1.Text + "' and '" + dateTimePicker2.Text + "') and dbo.InventoryTransferStore.branchid='" + cmbbranchjv.SelectedValue + "' GROUP BY dbo.RawItem.Id, dbo.RawItem.Code, dbo.RawItem.ItemName, dbo.RawItem.Description, dbo.UOM.UOM, dbo.Type.TypeName order by dbo.RawItem.ItemName";
             ...
             double TotalCost = 0;
                for (int k = 0; k < dsaccounts.Tables[0].Rows.Count; k++)
                {
                    try
                    {
                        ...
                        try
                        {
                            string temp = dsaccounts.Tables[0].Rows[k]["QuantityConsumed"].ToString();
                            if (temp == "")
                            {
                                temp = "0";
                            }
                            qty = Convert.ToDouble(temp);
                            price = getcostmenu1(dateTimePicker1.Text, dateTimePicker2.Text, dsaccounts.Tables[0].Rows[k]["rid"].ToString());
                            Amount = price * qty;
                            TotalCost += Amount;
              ...
              /*  ///////////////////////////////////////////// 
                ...
                qr = "SELECT   TOP (100) PERCENT SUM(dbo.InventoryTransfer.TransferIn) AS QuantityConsumed, dbo.RawItem.Id AS rid, dbo.RawItem.Code, dbo.RawItem.ItemName, dbo.RawItem.Description, dbo.UOM.UOM,    dbo.Type.TypeName FROM            dbo.InventoryTransfer INNER JOIN                         dbo.RawItem ON dbo.InventoryTransfer.ItemId = dbo.RawItem.Id INNER JOIN                         dbo.UOM ON dbo.RawItem.UOMId = dbo.UOM.Id INNER JOIN                        dbo.Type ON dbo.RawItem.TypeId = dbo.Type.Id WHERE     (dbo.InventoryTransfer.Date between '" + dateTimePicker1.Text + "' and '" + dateTimePicker2.Text + "') and dbo.InventoryTransferStore.branchid='" + cmbbranchjv.SelectedValue + "' GROUP BY dbo.RawItem.Id, dbo.RawItem.Code, dbo.RawItem.ItemName, dbo.RawItem.Description, dbo.UOM.UOM, dbo.Type.TypeName order by dbo.RawItem.ItemName";
            ...
            else
            {
                ...
                for (int k = 0; k < dsaccounts.Tables[0].Rows.Count; k++)
                {
                    try
                    {
                        ...
                        if (dsaccounts.Tables[0].Rows[k]["TypeName"].ToString().ToUpper() == "PACKING")
                        {
                            packingcost = packingcost + (consumed * price);
                        }
                        else
        ...
        public double getprice(string id)
        {
            ...
            double cost = 0;
            string q = "select  dbo.Getprice('" +Convert.ToDateTime(dateTimePicker1.Text) + "','" + Convert.ToDateTime(dateTimePicker2.Text) + "'," + id + ")";
            try
            {
                DataSet ds = new DataSet();
                ds = objCore.funGetDataSet(q);

...
{
    public partial class AttachSubRecipe : Form
    {
        ...
        public void getdata()
        {
            try
            {
                ...
                string q = "";
                if (cmbflavour.Text == "" || cmbflavour.Text == "Please Select")
                {
                    q = "SELECT        dbo.AttachRecipe.Id, dbo.MenuItem.Name, dbo.ModifierFlavour.name AS Flavour, dbo.SubItems.Name AS SubItem, dbo.AttachRecipe.Quantity FROM            dbo.AttachRecipe INNER JOIN                         dbo.MenuItem ON dbo.AttachRecipe.Menuitemid = dbo.MenuItem.Id INNER JOIN                         dbo.SubItems ON dbo.AttachRecipe.SubItemId = dbo.SubItems.Id LEFT OUTER JOIN                         dbo.ModifierFlavour ON dbo.AttachRecipe.FlavourId = dbo.ModifierFlavour.Id  where dbo.AttachRecipe.Menuitemid='" + cmbitem.SelectedValue + "'  and dbo.AttachRecipe.type='MenuItem'";
                }
                else
                {

                    q = "SELECT        dbo.AttachRecipe.Id, dbo.MenuItem.Name, dbo.ModifierFlavour.name AS Flavour, dbo.SubItems.Name AS SubItem, dbo.AttachRecipe.Quantity FROM            dbo.AttachRecipe INNER JOIN                         dbo.MenuItem ON dbo.AttachRecipe.Menuitemid = dbo.MenuItem.Id INNER JOIN                         dbo.SubItems ON dbo.AttachRecipe.SubItemId = dbo.SubItems.Id LEFT OUTER JOIN                         dbo.ModifierFlavour ON dbo.AttachRecipe.FlavourId = dbo.ModifierFlavour.Id where dbo.AttachRecipe.Menuitemid='" + cmbitem.SelectedValue + "' and dbo.AttachRecipe.flavourid='" + cmbflavour.SelectedValue + "'  and dbo.AttachRecipe.type='MenuItem'";
        ...
        public void fillflavour()
        {
            try
            {
                cmbflavour.DataSource = null;
                POSRestaurant.classes.Clsdbcon objCore = new classes.Clsdbcon();
                DataSet ds2 = new DataSet();
                string q = "select * from modifierflavour where menuitemid='" + cmbitem.SelectedValue + "' order by name";
                ds2 = objCore.funGetDataSet(q);

                cmbflavour.DataSource = ds2.Tables[0];
        ...
        private void AddGroups_Load(object sender, EventArgs e)
        {
            ...
            if (editmode == 1)
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    try
                    {
                        cmbflavour.SelectedValue = ds.Tables[0].Rows[0]["Flavourid"].ToString();
                    }
                    catch (Exception wx)
                    {
        ...
        public void getinfo(string id)
        {
            try
            {
                ...
                if (dsinfo.Tables[0].Rows.Count > 0)
                {
                    ...
                    try
                    {
                        cmbflavour.SelectedValue = dsinfo.Tables[0].Rows[0]["flavourid"].ToString();
                    }
                    catch (Exception ex)
                    {
        ...
        private void vButton2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (vButton2.Text == "Add")
                {
                    ...
                    ds = new DataSet();
                    string q = "";
                    q = "select * from AttachRecipe where SubItemId='" + comboBox3.SelectedValue + "' and Menuitemid='" + cmbitem.SelectedValue + "'  and type='MenuItem'";
                    ds = objCore.funGetDataSet(q);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                    ...
                    try
                    {
                        flavourid = cmbflavour.SelectedValue.ToString();
                    }
                    catch (Exception ex)
                    {
                    ...
                    q = "insert into AttachRecipe (FlavourId,SubItemId,Menuitemid,Quantity,Type) values('" + flavourid + "','" + comboBox3.SelectedValue + "','" + cmbitem.SelectedValue + "','" + richTextBox1.Text.Trim().Replace("'", "''") + "','MenuItem')";
                ...
                if (vButton2.Text == "Update")
                {
                    ...
                    string q = "";
                    q = "update AttachRecipe set  FlavourId='" + flavourid + "', Menuitemid='" + cmbitem.SelectedValue + "', SubItemId='" + comboBox3.SelectedValue + "' , Quantity ='" + richTextBox1.Text.Trim().Replace("'", "''") + "' where id='" + itemid + "'";
